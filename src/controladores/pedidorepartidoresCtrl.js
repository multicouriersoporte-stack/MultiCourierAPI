...
import { conmysql } from "../db.js";
import { notificationService } from "../notifications/notification.service.js";
import { filtrarRepartidoresPorEfectivo, validarRepartidorPuedeTomarPedido, ErrorFinanzas } from "../servicios/finanzasService.js";
...
// Bloquea el pedido; incluye id_metodo_pago para restricción de efectivo.
const obtenerPedidoBloqueado = async (conexion, id_pedido) => {
  const [pedidos] = await conexion.query(
    `SELECT p.id_metodo_pago,p.id_pedido,p.pedido_codigo,p.id_cliente,p.id_local,p.id_repartidor,p.id_estado,p.pedido_fecha,p.pedido_total,p.pedido_carrera,p.pedido_local_latitud,p.pedido_local_longitud,p.pedido_cliente_latitud,p.pedido_cliente_longitud,e.estado_nombre
     FROM pedidos p LEFT JOIN estados e ON p.id_estado=e.id_estado
     WHERE p.id_pedido=? LIMIT 1 FOR UPDATE`,
    [id_pedido]
  );
...
const liberarRepartidorSegunTurno = async (conexion, id_repartidor) => {
  if (!id_repartidor) return;
  const [turno] = await conexion.query(
    `SELECT hr.id_reserva FROM horario_reservas hr
     INNER JOIN horarios_disponibles hd ON hd.id_horario_disponible=hr.id_horario_disponible
     WHERE hr.id_repartidor=? AND hr.reserva_estado=1
  ...
  const idEstadoDestino = await obtenerIdEstadoRepartidorPorNombre(conexion, nombreDestino);
  if (idEstadoDestino) {
    await conexion.query(`UPDATE repartidores SET id_estado_repartidor=? WHERE id_repartidor=?`, [idEstadoDestino, id_repartidor]);
  }
};

// Asignación automática con filtro de efectivo.
export const asignarRepartidorAutomaticamente = async id_pedido => {
  const conexion = await conmysql.getConnection();
  try {
    await conexion.beginTransaction();
    ...
    // Prioriza pedidos simultáneos de mayor valor.
    const [pedidosSimultaneos] = await conexion.query(
      `SELECT p2.id_pedido,p2.pedido_codigo,p2.pedido_total,p2.pedido_fecha FROM pedidos p2
       WHERE p2.id_estado=? AND p2.id_repartidor IS NULL AND p2.id_pedido<>?
       AND p2.pedido_fecha BETWEEN DATE_SUB(?,INTERVAL ? MINUTE) AND DATE_ADD(?,INTERVAL ? MINUTE)
       ORDER BY p2.pedido_total DESC,p2.pedido_fecha ASC,p2.id_pedido ASC`,
    ...
    if (pedidoDeMayorValor) {
      await conexion.rollback();
      return {
        asignado: false, motivo: "Existe otro pedido simultáneo de mayor valor.",
        pedido_prioritario: { id_pedido: pedidoDeMayorValor.id_pedido, pedido_codigo: pedidoDeMayorValor.pedido_codigo, pedido_total: Number(pedidoDeMayorValor.pedido_total || 0) }
      };
    }
    ...
    const [repartidores] = await conexion.query(
      `SELECT r.id_repartidor,r.id_usuario,r.repartidor_codigo,r.repartidor_placa,r.repartidor_tipo_vehiculo,
       ...
       INNER JOIN (
           SELECT ru1.* FROM repartidor_ubicaciones ru1
           INNER JOIN (SELECT id_repartidor,MAX(id_repartidor_ubicacion) AS ultima_ubicacion FROM repartidor_ubicaciones GROUP BY id_repartidor) ultima
    ...
    // Restricción de efectivo: pedidos en efectivo excluyen repartidores sobre el límite.
    const { elegibles, excluidos } = await filtrarRepartidoresPorEfectivo(conexion, repartidores, pedido.id_metodo_pago);
    if (!elegibles.length) {
      await conexion.rollback();
      return {
        asignado: false,
        motivo: excluidos.length
          ? "Pedido en efectivo: todos los repartidores cercanos superaron su límite de efectivo."
          : "No existen repartidores LISTO o REPARTIENDO con ubicación reciente."
      };
    }
    ...
    let radioSeleccionado = null;
    for (const radio of RADIOS_ASIGNACION) {
      candidatos = elegibles.filter(r => Number(r.distancia_km) <= radio);
    ...
    candidatos.sort((a, b) => {
      const distanciaA = Number(a.distancia_km || 999999), distanciaB = Number(b.distancia_km || 999999);
      if (distanciaA !== distanciaB) return distanciaA - distanciaB;
      const pedidosA = Number(a.repartidor_total_pedidos || 0), pedidosB = Number(b.repartidor_total_pedidos || 0);
      if (Math.abs(pedidosA - pedidosB) <= DIFERENCIA_PEDIDOS_RANKING) {
        const rankingA = Number(a.repartidor_posicion_ranking ?? 999999), rankingB = Number(b.repartidor_posicion_ranking ?? 999999);
        if (rankingA !== rankingB) return rankingA - rankingB;
      }
      if (pedidosA !== pedidosB) return pedidosA - pedidosB;
      const puntosA = Number(a.repartidor_puntos || 0), puntosB = Number(b.repartidor_puntos || 0);
      if (puntosA !== puntosB) return puntosB - puntosA;
      const calificacionA = Number(a.repartidor_calificacion || 0), calificacionB = Number(b.repartidor_calificacion || 0);
      if (calificacionA !== calificacionB) return calificacionB - calificacionA;
      return Number(a.id_repartidor) - Number(b.id_repartidor);
    });
    ...
    const idPedidoRepartidor = await crearAsignacion({ conexion, pedido, repartidor, idEstadoOfertado });
    const actualizacion = await asociarRepartidorAlPedido(conexion, id_pedido, repartidor.id_repartidor);
    if (!actualizacion.affectedRows) {
      await conexion.query(`DELETE FROM pedido_repartidores WHERE id_pedido_repartidor=?`, [idPedidoRepartidor]);
      await conexion.rollback();
    ...
    return {
      asignado: true, tipo_asignacion: "AUTOMATICA", id_pedido, pedido_codigo: pedido.pedido_codigo,
      id_pedido_repartidor: idPedidoRepartidor, id_repartidor: repartidor.id_repartidor, repartidor_codigo: repartidor.repartidor_codigo,
      estado_pedido: ESTADO_PEDIDO_EN_PREPARACION, estado_asignacion: ESTADO_PR_OFERTADO, estado_repartidor: ESTADO_REPARTIDOR_EN_PEDIDO,
      distancia_km: Number(Number(candidato.distancia_km).toFixed(3)), radio_asignacion_km: radioSeleccionado,
      pedido_total: Number(pedido.pedido_total || 0), mensaje_repartidor: "Pedido asignado"
    };
  } catch (error) {
...
// Lista repartidores disponibles (normal o forzado). Incluye balance/límite de efectivo para la UI.
export const getRepartidoresDisponiblesAsignacion = async (req, res) => {
  try {
    if (!req.usuario) return res.status(401).json({ success: false, message: "Usuario no autenticado." });
    ...
    if (esIdValido(id_pedido)) {
      const [pedidos] = await conmysql.query(
        `SELECT id_pedido, pedido_local_latitud, pedido_local_longitud, id_repartidor FROM pedidos WHERE id_pedido = ? LIMIT 1`,
        [id_pedido]
      );
    ...
    let sql = `
      SELECT r.id_repartidor,r.id_usuario,r.repartidor_codigo,r.repartidor_placa,r.repartidor_tipo_vehiculo,
             r.repartidor_posicion_ranking,r.repartidor_total_pedidos,r.repartidor_puntos,r.repartidor_calificacion,
             r.id_estado_repartidor,r.repartidor_balance_efectivo,r.repartidor_limite_billetera,
             er.estado_repartidor_nombre,er.estado_repartidor_permite_pedidos,er.estado_repartidor_permite_seleccion,
             u.usuario_nombre_completo,u.usuario_telefono,u.usuario_foto,
             ru.repartidor_ubicacion_latitud,ru.repartidor_ubicacion_longitud,ru.repartidor_ubicacion_fecha`;
    ...
    // Normal → solo LISTO/REPARTIENDO; forzado → todos los estados.
    if (!esForzado) {
      sql += ` AND UPPER(TRIM(er.estado_repartidor_nombre)) IN ('LISTO', 'REPARTIENDO') AND er.estado_repartidor_permite_pedidos = 1`;
    }
    ...
    sql += `
      ORDER BY
        CASE
          ...
          ELSE 5
        END,
        distancia_km ASC, r.repartidor_posicion_ranking ASC, r.repartidor_total_pedidos ASC,
        r.repartidor_puntos DESC, r.repartidor_calificacion DESC, r.id_repartidor ASC`;

    const [rows] = await conmysql.query(sql, parametros);
    let repartidores = rows;
    if (esIdValido(id_pedido)) {
      const [ped] = await conmysql.query(`SELECT id_repartidor FROM pedidos WHERE id_pedido = ? LIMIT 1`, [id_pedido]);
      const idActual = ped[0]?.id_repartidor;
      if (idActual != null) repartidores = rows.filter(r => Number(r.id_repartidor) !== Number(idActual));
    }

    return res.json({ success: true, forzado: esForzado, total: repartidores.length, repartidores });
  } catch (error) {
    console.error("[PedidoRepartidor] Error repartidores disponibles:", error);
...
// Asignación manual (valida límite de efectivo).
export const asignarRepartidorManualmente = async (req, res) => {
  const conexion = await conmysql.getConnection();
  try {
    if (!req.usuario) return res.status(401).json({ success: false, message: "Usuario no autenticado." });
    ...
    if (!pedidoPermiteAsignacion(pedido.estado_nombre)) {
      await conexion.rollback();
      return res.status(409).json({ success: false, message: `No se puede asignar un pedido en estado ${pedido.estado_nombre}.`, estado_actual: pedido.estado_nombre });
    }
    if (pedido.id_repartidor !== null) {
      await conexion.rollback();
      return res.status(409).json({ success: false, message: "El pedido ya tiene un repartidor. Para cambiarlo usa la reasignación." });
    }
    ...
    // Restricción de efectivo (también en forzada; override solo si se implementa ignorar_limite_efectivo).
    await validarRepartidorPuedeTomarPedido(conexion, { id_repartidor, id_metodo_pago: pedido.id_metodo_pago });
    ...
    return res.status(201).json({
      success: true, message: "Repartidor asignado manualmente correctamente.", tipo_asignacion: "MANUAL",
      id_pedido, pedido_codigo: pedido.pedido_codigo, id_repartidor, repartidor_codigo: repartidor.repartidor_codigo,
      id_pedido_repartidor: idPedidoRepartidor, estado_pedido: pedido.estado_nombre,
      estado_asignacion: ESTADO_PR_OFERTADO, estado_repartidor: ESTADO_REPARTIDOR_EN_PEDIDO
    });
  } catch (error) {
...
// Asignación forzada: ignora estado del repartidor, no el límite de efectivo.
export const asignarRepartidorForzado = async (req, res) => {
  const conexion = await conmysql.getConnection();
  try {
    if (!req.usuario) return res.status(401).json({ success: false, message: "Usuario no autenticado." });
    ...
    return res.status(201).json({
      success: true, message: "Repartidor asignado forzadamente.", tipo_asignacion: "MANUAL_FORZADA",
      id_pedido, pedido_codigo: pedido.pedido_codigo, id_repartidor, repartidor_codigo: repartidor.repartidor_codigo,
      estado_repartidor_previo: repartidor.estado_repartidor_nombre, id_pedido_repartidor: idPedidoRepartidor, motivo
    });
  } catch (error) {
    if (error instanceof ErrorFinanzas) {
      try { await conexion.rollback(); } catch (_) {}
...
// Reasignación forzada (valida límite de efectivo).
export const reasignarRepartidorForzado = async (req, res) => {
  const conexion = await conmysql.getConnection();
  try {
    if (!req.usuario) return res.status(401).json({ success: false, message: "Usuario no autenticado." });
    ...
    if (!pedidoPermiteAsignacion(estadoPedido)) {
      await conexion.rollback();
      return res.status(409).json({ success: false, message: `No se puede reasignar un pedido en estado ${pedido.estado_nombre}.`, estado_actual: pedido.estado_nombre });
    }
    ...
    return res.status(200).json({
      success: true, message: "Repartidor reasignado forzadamente.", tipo_asignacion: "REASIGNACION_FORZADA",
      id_pedido, pedido_codigo: pedido.pedido_codigo,
      repartidor_anterior: asignacionAnterior
        ? { id_repartidor: asignacionAnterior.id_repartidor, repartidor_codigo: asignacionAnterior.repartidor_codigo }
        : idRepartidorAnterior ? { id_repartidor: idRepartidorAnterior } : null,
      nuevo_repartidor: { id_repartidor, repartidor_codigo: repartidor.repartidor_codigo, estado_repartidor_previo: repartidor.estado_repartidor_nombre },
      id_pedido_repartidor: nuevoIdPedidoRepartidor, motivo
    });
  } catch (error) {
    if (error instanceof ErrorFinanzas) {
      try { await conexion.rollback(); } catch (_) {}
...
// Reasignación normal (valida límite de efectivo).
export const reasignarRepartidor = async (req, res) => {
  const conexion = await conmysql.getConnection();
  try {
    if (!req.usuario) return res.status(401).json({ success: false, message: "Usuario no autenticado." });
    ...
    return res.status(200).json({
      success: true, message: "Repartidor reasignado correctamente.", tipo_asignacion: "REASIGNACION",
      id_pedido, pedido_codigo: pedido.pedido_codigo,
      repartidor_anterior: asignacionAnterior
        ? { id_repartidor: asignacionAnterior.id_repartidor, repartidor_codigo: asignacionAnterior.repartidor_codigo, id_pedido_repartidor: asignacionAnterior.id_pedido_repartidor }
        : idRepartidorAnterior ? { id_repartidor: idRepartidorAnterior } : null,
      nuevo_repartidor: { id_repartidor, repartidor_codigo: repartidor.repartidor_codigo, estado_repartidor: ESTADO_REPARTIDOR_EN_PEDIDO },
...
export {
  obtenerIdEstadoPedido, obtenerIdEstadoPedidoRepartidor, obtenerIdUsuario, obtenerRol, tieneRol,
  puedeAsignarManualmente, puedeReasignar, esIdValido, repartidorPuedeRecibirPedido,
  obtenerAsignacionActiva, obtenerAsignacionesPedido, obtenerIdEstadoRepartidorPorNombre,
  marcarRepartidorEnPedido, liberarRepartidorSegunTurno
};

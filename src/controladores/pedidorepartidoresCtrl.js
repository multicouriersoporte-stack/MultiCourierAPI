...
const obtenerIdEstadoPedido = async (conexion, nombreEstado) => {
    const [estados] = await conexion.query(`SELECT id_estado FROM estados WHERE estado_tipo='PEDIDO' AND UPPER(TRIM(estado_nombre))=? AND estado_activo=1 LIMIT 1`, [String(nombreEstado).trim().toUpperCase()]);
    return estados.length ? estados[0].id_estado : null;
};
const obtenerIdEstadoPedidoRepartidor = async (conexion, nombreEstado) => {
    const [estados] = await conexion.query(`SELECT id_estado FROM estados WHERE estado_tipo='PEDIDO_REPARTIDOR' AND UPPER(TRIM(estado_nombre))=? AND estado_activo=1 LIMIT 1`, [String(nombreEstado).trim().toUpperCase()]);
    return estados.length ? estados[0].id_estado : null;
};
// Resuelve el estado del repartidor por nombre.
const obtenerIdEstadoRepartidorPorNombre = async (conexion, nombre) => {
    const [rows] = await conexion.query(`SELECT id_estado_repartidor FROM estados_repartidor WHERE UPPER(TRIM(estado_repartidor_nombre))=? AND estado_repartidor_estado=1 LIMIT 1`, [String(nombre).trim().toUpperCase()]);
...
const obtenerPedidoBloqueado = async (conexion, id_pedido) => {
    const [pedidos] = await conexion.query(`SELECT p.id_pedido,p.pedido_codigo,p.id_cliente,p.id_local,p.id_repartidor,p.id_estado,p.pedido_fecha,p.pedido_total,p.pedido_carrera,p.pedido_local_latitud,p.pedido_local_longitud,p.pedido_cliente_latitud,p.pedido_cliente_longitud,e.estado_nombre FROM pedidos p LEFT JOIN estados e ON p.id_estado=e.id_estado WHERE p.id_pedido=? LIMIT 1 FOR UPDATE`, [id_pedido]);
...
const obtenerRepartidorBloqueado = async (conexion, id_repartidor) => {
    const [rows] = await conexion.query(`SELECT r.id_repartidor,r.id_usuario,r.repartidor_codigo,r.repartidor_placa,r.repartidor_tipo_vehiculo,r.repartidor_posicion_ranking,r.repartidor_total_pedidos,r.repartidor_puntos,r.repartidor_calificacion,r.id_estado_repartidor,er.estado_repartidor_nombre,er.estado_repartidor_permite_pedidos,er.estado_repartidor_permite_seleccion,er.estado_repartidor_estado FROM repartidores r INNER JOIN estados_repartidor er ON r.id_estado_repartidor=er.id_estado_repartidor WHERE r.id_repartidor=? LIMIT 1 FOR UPDATE`, [id_repartidor]);
...
const repartidorPuedeRecibirPedido = repartidor => {
    ...
    return ESTADOS_REPARTIDOR_DISPONIBLES.includes(estado) && Number(repartidor.estado_repartidor_permite_pedidos) === 1 && Number(repartidor.estado_repartidor_estado) === 1;
};
// Asignaciones activas.
const obtenerAsignacionActiva = async (conexion, id_pedido) => {
    const [rows] = await conexion.query(`SELECT pr.*,r.repartidor_codigo,e.estado_nombre FROM pedido_repartidores pr INNER JOIN repartidores r ON pr.id_repartidor=r.id_repartidor INNER JOIN estados e ON pr.id_estado=e.id_estado WHERE pr.id_pedido=? AND e.estado_tipo='PEDIDO_REPARTIDOR' AND UPPER(TRIM(e.estado_nombre)) IN ('OFERTADO','ACEPTADO','RECOGIDO') ORDER BY pr.id_pedido_repartidor DESC LIMIT 1 FOR UPDATE`, [id_pedido]);
    return rows.length ? rows[0] : null;
};
// Historial completo de asignaciones.
const obtenerAsignacionesPedido = async (conexion, id_pedido) => {
    const [rows] = await conexion.query(`SELECT pr.*,r.repartidor_codigo,r.repartidor_placa,r.repartidor_tipo_vehiculo,e.estado_nombre AS estado_asignacion FROM pedido_repartidores pr INNER JOIN repartidores r ON pr.id_repartidor=r.id_repartidor INNER JOIN estados e ON pr.id_estado=e.id_estado WHERE pr.id_pedido=? ORDER BY pr.id_pedido_repartidor DESC`, [id_pedido]);
    return rows;
};
// Crea una oferta para el repartidor.
const crearAsignacion = async ({ conexion, pedido, repartidor, idEstadoOfertado }) => {
    const [insert] = await conexion.query(`INSERT INTO pedido_repartidores (id_pedido,id_repartidor,pedido_repartidor_fecha_solicitud,pedido_repartidor_fecha_asignacion,pedido_repartidor_prioridad,pedido_repartidor_es_seleccion_cliente,pedido_repartidor_costo_seleccion,pedido_repartidor_orden_simultaneo,id_estado) VALUES (?,?,NOW(),NOW(),0,0,0,1,?)`, [pedido.id_pedido, repartidor.id_repartidor, idEstadoOfertado]);
...
const asociarRepartidorAlPedido = async (conexion, id_pedido, id_repartidor, idEstadoEnPreparacion) => {
    const [resultado] = await conexion.query(`UPDATE pedidos SET id_repartidor=? WHERE id_pedido=? AND id_repartidor IS NULL AND id_estado=?`, [id_repartidor, id_pedido, idEstadoEnPreparacion]);
    return resultado;
};
...
const marcarRepartidorEnPedido = async (conexion, id_repartidor) => {
    ...
    await conexion.query(`UPDATE repartidores SET id_estado_repartidor=? WHERE id_repartidor=?`, [idEstadoEnPedido, id_repartidor]);
};
...
const liberarRepartidorSegunTurno = async (conexion, id_repartidor) => {
    ...
    const [turno] = await conexion.query(`SELECT hr.id_reserva FROM horario_reservas hr INNER JOIN horarios_disponibles hd ON hd.id_horario_disponible=hr.id_horario_disponible WHERE hr.id_repartidor=? AND hr.reserva_estado=1 AND hd.horario_fecha=CURDATE() AND CURTIME() BETWEEN hd.horario_hora_inicio AND hd.horario_hora_fin LIMIT 1`, [id_repartidor]);
    ...
    if (idEstadoDestino) await conexion.query(`UPDATE repartidores SET id_estado_repartidor=? WHERE id_repartidor=?`, [idEstadoDestino, id_repartidor]);
};
...
export const asignarRepartidorAutomaticamente = async id_pedido => {
    ...
    try {
        ...
        if (asignacionActiva) {
            await conexion.rollback();
            return { asignado: false, motivo: "El pedido ya tiene una asignación activa.", id_repartidor: asignacionActiva.id_repartidor, id_pedido_repartidor: asignacionActiva.id_pedido_repartidor };
        }
        // Prioriza pedidos simultáneos de mayor valor.
        const [pedidosSimultaneos] = await conexion.query(`SELECT p2.id_pedido,p2.pedido_codigo,p2.pedido_total,p2.pedido_fecha FROM pedidos p2 WHERE p2.id_estado=? AND p2.id_repartidor IS NULL AND p2.id_pedido<>? AND p2.pedido_fecha BETWEEN DATE_SUB(?,INTERVAL ? MINUTE) AND DATE_ADD(?,INTERVAL ? MINUTE) ORDER BY p2.pedido_total DESC,p2.pedido_fecha ASC,p2.id_pedido ASC`, [idEstadoEnPreparacion, id_pedido, pedido.pedido_fecha, VENTANA_PEDIDOS_SIMULTANEOS_MINUTOS, pedido.pedido_fecha, VENTANA_PEDIDOS_SIMULTANEOS_MINUTOS]);
        ...
        if (pedidoDeMayorValor) {
            await conexion.rollback();
            return { asignado: false, motivo: "Existe otro pedido simultáneo de mayor valor.", pedido_prioritario: { id_pedido: pedidoDeMayorValor.id_pedido, pedido_codigo: pedidoDeMayorValor.pedido_codigo, pedido_total: Number(pedidoDeMayorValor.pedido_total || 0) } };
        }
        // Busca repartidores disponibles con ubicación reciente.
        const [repartidores] = await conexion.query(`SELECT r.id_repartidor,r.id_usuario,r.repartidor_codigo,r.repartidor_placa,r.repartidor_tipo_vehiculo,r.repartidor_posicion_ranking,r.repartidor_total_pedidos,r.repartidor_puntos,r.repartidor_calificacion,er.estado_repartidor_nombre,er.estado_repartidor_permite_pedidos,er.estado_repartidor_permite_seleccion,ru.repartidor_ubicacion_latitud,ru.repartidor_ubicacion_longitud,ru.repartidor_ubicacion_fecha,(6371*ACOS(LEAST(1,GREATEST(-1,COS(RADIANS(?))*COS(RADIANS(ru.repartidor_ubicacion_latitud))*COS(RADIANS(ru.repartidor_ubicacion_longitud)-RADIANS(?))+SIN(RADIANS(?))*SIN(RADIANS(ru.repartidor_ubicacion_latitud)))))) AS distancia_km FROM repartidores r INNER JOIN estados_repartidor er ON r.id_estado_repartidor=er.id_estado_repartidor INNER JOIN (SELECT ru1.* FROM repartidor_ubicaciones ru1 INNER JOIN (SELECT id_repartidor,MAX(id_repartidor_ubicacion) AS ultima_ubicacion FROM repartidor_ubicaciones GROUP BY id_repartidor) ultima ON ru1.id_repartidor_ubicacion=ultima.ultima_ubicacion) ru ON r.id_repartidor=ru.id_repartidor WHERE er.estado_repartidor_estado=1 AND UPPER(TRIM(er.estado_repartidor_nombre)) IN ('LISTO','REPARTIENDO') AND er.estado_repartidor_permite_pedidos=1 AND ru.repartidor_ubicacion_fecha>=DATE_SUB(NOW(),INTERVAL ? MINUTE)`, [pedido.pedido_local_latitud, pedido.pedido_local_longitud, pedido.pedido_local_latitud, MAX_ANTIGUEDAD_UBICACION_MINUTOS]);
        ...
        for (const radio of RADIOS_ASIGNACION) {
            ...
            if (candidatos.length) { radioSeleccionado = radio; break; }
        }
        ...
        return { asignado: true, tipo_asignacion: "AUTOMATICA", id_pedido, pedido_codigo: pedido.pedido_codigo, id_pedido_repartidor: idPedidoRepartidor, id_repartidor: repartidor.id_repartidor, repartidor_codigo: repartidor.repartidor_codigo, estado_pedido: ESTADO_PEDIDO_EN_PREPARACION, estado_asignacion: ESTADO_PR_OFERTADO, estado_repartidor: ESTADO_REPARTIDOR_EN_PEDIDO, distancia_km: Number(Number(candidato.distancia_km).toFixed(3)), radio_asignacion_km: radioSeleccionado, pedido_total: Number(pedido.pedido_total || 0), mensaje_repartidor: "Pedido asignado" };
    } catch (error) {
        try { await conexion.rollback(); } catch (rollbackError) { console.error("[PedidoRepartidor] Error rollback:", rollbackError); }
        console.error("[PedidoRepartidor] Error asignación automática:", error);
        throw error;
    } finally { conexion.release(); }
};
// Lista repartidores disponibles para asignación manual (soporta forzado=true para listar todos).
export const getRepartidoresDisponiblesAsignacion = async (req, res) => {
    try {
        ...
        const esForzado = String(forzado).toLowerCase() === "true" || forzado === "1";
        let lat = Number(latitud);
        let lng = Number(longitud);
        if (esIdValido(id_pedido)) {
            const [pedidos] = await conmysql.query(`SELECT id_pedido,pedido_local_latitud,pedido_local_longitud FROM pedidos WHERE id_pedido=? LIMIT 1`, [id_pedido]);
        ...
        const parametros = [];
        let sql = `SELECT r.id_repartidor,r.id_usuario,r.repartidor_codigo,r.repartidor_placa,r.repartidor_tipo_vehiculo,r.repartidor_posicion_ranking,r.repartidor_total_pedidos,r.repartidor_puntos,r.repartidor_calificacion,er.estado_repartidor_nombre,er.estado_repartidor_permite_pedidos,er.estado_repartidor_permite_seleccion,ru.repartidor_ubicacion_latitud,ru.repartidor_ubicacion_longitud,ru.repartidor_ubicacion_fecha`;
        if (tieneCoordenadas) {
            sql += `,(6371*ACOS(LEAST(1,GREATEST(-1,COS(RADIANS(?))*COS(RADIANS(ru.repartidor_ubicacion_latitud))*COS(RADIANS(ru.repartidor_ubicacion_longitud)-RADIANS(?))+SIN(RADIANS(?))*SIN(RADIANS(ru.repartidor_ubicacion_latitud)))))) AS distancia_km`;
        ...
        sql += `
            ...
            ${esForzado ? "" : `WHERE er.estado_repartidor_estado=1 AND UPPER(TRIM(er.estado_repartidor_nombre)) IN ('LISTO','REPARTIENDO') AND er.estado_repartidor_permite_pedidos=1`}
            ORDER BY CASE
        ...
        return res.json({ success: true, forzado: esForzado, repartidores: rows });
    } catch (error) {
...
export const asignarRepartidorManualmente = async (req, res) => {
    ...
    try {
        ...
        if (Number(pedido.id_estado) !== Number(idEstadoEnPreparacion)) {
            await conexion.rollback();
            return res.status(409).json({ success: false, message: `El pedido debe estar en ${ESTADO_PEDIDO_EN_PREPARACION}.`, estado_actual: pedido.estado_nombre, id_estado_actual: pedido.id_estado });
        }
        if (pedido.id_repartidor !== null) {
            await conexion.rollback();
        ...
        if (!repartidor) { await conexion.rollback(); return res.status(404).json({ success: false, message: "El repartidor no existe." }); }
        if (!repartidorPuedeRecibirPedido(repartidor)) {
            await conexion.rollback();
            return res.status(409).json({ success: false, message: "El repartidor seleccionado no puede recibir pedidos.", estado_repartidor: repartidor.estado_repartidor_nombre });
        }
        const idPedidoRepartidor = await crearAsignacion({ conexion, pedido, repartidor, idEstadoOfertado });
        ...
        await conexion.commit();
        return res.status(201).json({ success: true, message: "Repartidor asignado manualmente correctamente.", tipo_asignacion: "MANUAL", id_pedido, pedido_codigo: pedido.pedido_codigo, id_repartidor, repartidor_codigo: repartidor.repartidor_codigo, id_pedido_repartidor: idPedidoRepartidor, estado_pedido: ESTADO_PEDIDO_EN_PREPARACION, estado_asignacion: ESTADO_PR_OFERTADO, estado_repartidor: ESTADO_REPARTIDOR_EN_PEDIDO });
    } catch (error) {
        try { await conexion.rollback(); } catch (rollbackError) { console.error("[PedidoRepartidor] Error rollback:", rollbackError); }
        console.error("[PedidoRepartidor] Error asignación manual:", error);
        return res.status(500).json({ success: false, message: "Error al asignar repartidor manualmente.", error: process.env.NODE_ENV === "development" ? error.message : undefined });
    } finally { conexion.release(); }
};
...
export const asignarRepartidorForzado = async (req, res) => {
    ...
    try {
        ...
        // Sin bloqueo por estados: se acepta cualquier estado del repartidor.
        const [repartidores] = await conexion.query(`SELECT r.id_repartidor,r.repartidor_codigo,er.estado_repartidor_nombre FROM repartidores r INNER JOIN estados_repartidor er ON r.id_estado_repartidor=er.id_estado_repartidor WHERE r.id_repartidor=? LIMIT 1 FOR UPDATE`, [id_repartidor]);
        ...
        await marcarRepartidorEnPedido(conexion, id_repartidor);
        await conexion.commit();
        return res.status(201).json({ success: true, message: "Repartidor asignado forzadamente.", tipo_asignacion: "MANUAL_FORZADA", id_pedido, pedido_codigo: pedido.pedido_codigo, id_repartidor, repartidor_codigo: repartidor.repartidor_codigo, estado_repartidor_previo: repartidor.estado_repartidor_nombre, id_pedido_repartidor: idPedidoRepartidor, motivo });
    } catch (error) {
...
export const reasignarRepartidorForzado = async (req, res) => {
    ...
    try {
        ...
        await marcarRepartidorEnPedido(conexion, id_repartidor);
        await conexion.commit();
        return res.status(200).json({ success: true, message: "Repartidor reasignado forzadamente.", tipo_asignacion: "REASIGNACION_FORZADA", id_pedido, pedido_codigo: pedido.pedido_codigo, repartidor_anterior: asignacionAnterior ? { id_repartidor: asignacionAnterior.id_repartidor, repartidor_codigo: asignacionAnterior.repartidor_codigo } : idRepartidorAnterior ? { id_repartidor: idRepartidorAnterior } : null, nuevo_repartidor: { id_repartidor, repartidor_codigo: repartidor.repartidor_codigo, estado_repartidor_previo: repartidor.estado_repartidor_nombre }, id_pedido_repartidor: nuevoIdPedidoRepartidor, motivo });
    } catch (error) {
        try { await conexion.rollback(); } catch (e) { console.error("[PedidoRepartidor] Error rollback:", e); }
...
export const reasignarRepartidor = async (req, res) => {
    ...
    try {
        ...
        if (estadoPedido !== ESTADO_PEDIDO_EN_PREPARACION) {
            await conexion.rollback();
            return res.status(409).json({ success: false, message: `La reasignación solo está disponible mientras el pedido está en ${ESTADO_PEDIDO_EN_PREPARACION}.`, estado_actual: pedido.estado_nombre });
        }
        const idRepartidorAnterior = pedido.id_repartidor;
        const repartidor = await obtenerRepartidorBloqueado(conexion, id_repartidor);
        if (!repartidor) { await conexion.rollback(); return res.status(404).json({ success: false, message: "El nuevo repartidor no existe." }); }
        if (!repartidorPuedeRecibirPedido(repartidor)) {
            await conexion.rollback();
            return res.status(409).json({ success: false, message: "El nuevo repartidor no puede recibir pedidos.", estado_repartidor: repartidor.estado_repartidor_nombre });
        }
        if (Number(idRepartidorAnterior) === Number(id_repartidor)) {
            await conexion.rollback();
            return res.status(409).json({ success: false, message: "El repartidor seleccionado ya es el repartidor actual." });
        ...
        await marcarRepartidorEnPedido(conexion, id_repartidor);
        await conexion.commit();
        return res.status(200).json({ success: true, message: "Repartidor reasignado correctamente.", tipo_asignacion: "REASIGNACION", id_pedido, pedido_codigo: pedido.pedido_codigo, repartidor_anterior: asignacionAnterior ? { id_repartidor: asignacionAnterior.id_repartidor, repartidor_codigo: asignacionAnterior.repartidor_codigo, id_pedido_repartidor: asignacionAnterior.id_pedido_repartidor } : idRepartidorAnterior ? { id_repartidor: idRepartidorAnterior } : null, nuevo_repartidor: { id_repartidor, repartidor_codigo: repartidor.repartidor_codigo, estado_repartidor: ESTADO_REPARTIDOR_EN_PEDIDO }, id_pedido_repartidor: nuevoIdPedidoRepartidor, estado_pedido: estadoPedido, estado_asignacion: ESTADO_PR_OFERTADO, motivo });
    } catch (error) {
        try { await conexion.rollback(); } catch (rollbackError) { console.error("[PedidoRepartidor] Error rollback:", rollbackError); }
        console.error("[PedidoRepartidor] Error reasignación:", error);
        return res.status(500).json({ success: false, message: "Error al reasignar repartidor.", error: process.env.NODE_ENV === "development" ? error.message : undefined });
    } finally { conexion.release(); }
};
...
export const getAsignacionesPedido = async (req, res) => {
    try {
        if (!req.usuario) return res.status(401).json({ success: false, message: "Usuario no autenticado." });
        if (!tieneRol(req, ["ADMINISTRADOR", "CENTRAL", "SOPORTE", "LOCAL"])) return res.status(403).json({ success: false, message: "No tienes permisos para consultar las asignaciones del pedido." });
        const id_pedido = Number(req.params.id_pedido);
        if (!esIdValido(id_pedido)) return res.status(400).json({ success: false, message: "El ID del pedido no es válido." });

...
const tieneRol = (req, rolesPermitidos = []) =>
    rolesPermitidos.map(r => String(r).trim().toUpperCase()).includes(obtenerRol(req));
...
/**
 * Obtiene el repartidor asociado a un usuario.
 */
...
/**
 * Obtiene un pago con información del repartidor y pedido.
 */
const obtenerPagoPorIdInterno = async id_pago_repartidor => {
    const [rows] = await conmysql.query(`
        SELECT
            ...
            p.pedido_propina, p.pedido_total,
...
/**
 * Crea automáticamente el pago cuando el pedido está ENTREGADO (15).
 * El repartidor recibe 95% de la carrera + 100% de la propina.
 * La operación es idempotente para evitar pagos duplicados.
 */
export const crearPagoRepartidorDesdePedido = async (id_pedido, conexion = conmysql) => {
    ...
    // Obtiene los datos necesarios del pedido.
    ...
    if (!pedido.id_repartidor) {
        throw new Error(`El pedido ${idPedido} no tiene un repartidor asociado.`);
    }

    // Solo se genera cuando el pedido está ENTREGADO.
    ...
    if (!Number.isFinite(carrera) || carrera < 0) {
        throw new Error(`El pedido ${idPedido} tiene una carrera inválida.`);
    }

    if (!Number.isFinite(propina) || propina < 0) {
        throw new Error(`El pedido ${idPedido} tiene una propina inválida.`);
    }
    ...
    const [resultado] = await conexion.query(`
        INSERT INTO pagos_repartidor (
            ...
            pago_repartidor_carrera, pago_repartidor_propina,
            pago_repartidor_otros, pago_repartidor_total,
            pago_repartidor_estado, pago_repartidor_fecha_pago
        )
        VALUES (?, ?, NOW(), ?, ?, ?, ?, ?, NULL)
    `, [
        pedido.id_repartidor, pedido.id_pedido, carreraNeta,
        propinaRepartidor, otros, total, "PENDIENTE"
    ]);
...
/**
 * Obtiene todos los pagos según el rol del usuario.
 */
export const getPagosRepartidor = async (req, res) => {
    try {
        ...
        return res.status(403).json({
            success: false,
            message: "No tienes permisos para consultar pagos del repartidor."
        });
    } catch (error) {
        console.error("[PagosRepartidor] Error getPagosRepartidor:", error);
        return res.status(500).json({
            success: false,
            message: "Error al consultar pagos del repartidor",
...
export const getPagoRepartidorxid = async (req, res) => {
    try {
        ...
        if (tieneRol(req, ["REPARTIDOR"])) {
            ...
            if (Number(pago.id_repartidor) !== Number(repartidor.id_repartidor)) {
                return res.status(403).json({
                    success: false,
                    message: "No puedes consultar el pago de otro repartidor."
                });
            }
...
export const getPagosRepartidorPorRepartidor = async (req, res) => {
    try {
        ...
        if (tieneRol(req, ["REPARTIDOR"])) {
            ...
            if (Number(repartidor.id_repartidor) !== Number(id_repartidor)) {
                return res.status(403).json({
                    success: false,
                    message: "No puedes consultar pagos de otro repartidor."
                });
            }
        ...
        return res.status(403).json({
            success: false,
            message: "No tienes permisos para consultar estos pagos."
        });
    } catch (error) {
...
export const getPagosRepartidorPorPedido = async (req, res) => {
    try {
        ...
        if (tieneRol(req, ["REPARTIDOR"])) {
            ...
            return res.json(rows.filter(pago =>
                Number(pago.id_repartidor) === Number(repartidor.id_repartidor)
            ));
        }

        return res.status(403).json({
            success: false,
            message: "No tienes permisos para consultar pagos por pedido."
        });
    } catch (error) {
        console.error("[PagosRepartidor] Error getPagosRepartidorPorPedido:", error);
        return res.status(500).json({
            success: false,
            message: "Error al consultar el pago del pedido"
        });
    }
};
...
export const postPagosRepartidor = async (req, res) => {
    try {
        ...
        if (!esAdministrativo(req)) {
            return res.status(403).json({
                success: false,
                message: "Solo ADMINISTRADOR y CENTRAL pueden crear pagos."
            });
        }
        ...
        if (!esIdValido(id_repartidor)) {
            return res.status(400).json({
                success: false,
                message: "El ID del repartidor es obligatorio y válido."
            });
        }
        ...
        if (!esIdValido(id_pedido)) {
            return res.status(400).json({
                success: false,
                message: "El ID del pedido es obligatorio y válido."
            });
        }
        ...
        if (existentes.length) {
            return res.status(409).json({
                success: false,
                message: "El pedido ya tiene un pago registrado para el repartidor.",
                pago: existentes[0]
            });
        }
        ...
        if (![carrera, propina, otros, total].every(Number.isFinite) ||
            [carrera, propina, otros, total].some(v => v < 0)) {
            return res.status(400).json({
                success: false,
                message: "Los valores del pago no son válidos."
            });
        }
        ...
        const [resultado] = await conmysql.query(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [
            id_repartidor, id_pedido, pago_repartidor_fecha || new Date(),
            carrera, propina, otros, total, estadoPago,
            pago_repartidor_fecha_pago || null
        ]);
        ...
        return res.status(201).json({
            success: true,
            message: "Pago del repartidor creado correctamente.",
            pago
        });
    } catch (error) {
        ...
        return res.status(500).json({
            success: false,
            message: "Error al crear el pago del repartidor",
            error: process.env.NODE_ENV === "development" ? error.message : undefined
        });
    }
};
...
export const putPagosRepartidor = async (req, res) => {
    try {
        ...
        if (!esAdministrativo(req)) {
            return res.status(403).json({
                success: false,
                message: "Solo ADMINISTRADOR y CENTRAL pueden modificar pagos."
            });
        }
        ...
        if (!pago) {
            return res.status(404).json({
                success: false,
                message: "Pago del repartidor no encontrado."
            });
        }
        ...
        // Valida IDs y valores monetarios.
        const camposNumericos = [
            "id_repartidor", "id_pedido", "pago_repartidor_carrera",
            "pago_repartidor_propina", "pago_repartidor_otros",
        ...
        for (const campo of camposNumericos) {
            ...
            const invalido = !Number.isFinite(valor) ||
                (campo.startsWith("id_") && valor <= 0) ||
                (!campo.startsWith("id_") && valor < 0);

            if (invalido) {
                return res.status(400).json({
                    success: false,
                    message: `El valor de ${campo} no es válido.`
                });
            }
        }
        ...
        return res.json({
            success: true,
            message: "Pago del repartidor actualizado correctamente.",
        ...
        return res.status(500).json({
            success: false,
            message: "Error al actualizar el pago del repartidor"
        });
    }
};
...
export const deletePagosRepartidor = async (req, res) => {
    try {
        ...
        if (!tieneRol(req, ["ADMINISTRADOR"])) {
            return res.status(403).json({
                success: false,
                message: "Solo ADMINISTRADOR puede eliminar pagos del repartidor."
            });
        }
        ...
        return res.status(500).json({
            success: false,
            message: "Error al eliminar el pago del repartidor"
        });
    }
};

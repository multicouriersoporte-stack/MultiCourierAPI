...
// Obtiene un pedido y sus datos relacionados.
...
// Busca un pago existente para evitar duplicados.
...
// Crea el pago del repartidor a partir de un pedido entregado.
export const crearPagoRepartidorDesdePedido = async id_pedido => {
  ...
  try {
    ...
    const pedido = await obtenerPedidoParaPago(conexion, id_pedido, true);
    if (!pedido) throw new Error("El pedido no existe.");

    const estadoPedido = String(pedido.estado_nombre || "").trim().toUpperCase();
    if (estadoPedido !== "ENTREGADO") {
      throw new Error(`No se puede generar el pago del repartidor porque el pedido está en estado ${estadoPedido || "DESCONOCIDO"}.`);
    }

    if (!pedido.id_repartidor) throw new Error("El pedido no tiene un repartidor asignado.");

    const pagoExistente = await obtenerPagoExistente(conexion, id_pedido, true);
    if (pagoExistente) {
      await conexion.commit();
      return { success: true, existente: true, creado: false, mensaje: "El pago del repartidor ya existe.", pago: pagoExistente };
    }

    // El repartidor recibe carrera + propina.
    const carrera = Number(pedido.pedido_carrera || 0);
    const propina = Number(pedido.pedido_propina || 0);
    const monto = carrera + propina;
    ...
    `, [id_pedido, pedido.id_repartidor, monto]);
...
// Crea manualmente el pago de un pedido.
...
export const getPagoRepartidorPorPedido = async (req, res) => {
  try {
    ...
    const [rows] = await conmysql.query(`
      SELECT pr.*, p.pedido_codigo, p.id_repartidor, r.repartidor_codigo,
             u.usuario_nombre, u.usuario_apellido, u.usuario_nombre_completo,
    ...
    if (!rows.length) {
      return res.status(404).json({ success: false, message: "El pedido todavía no tiene un pago para el repartidor." });
    }
...
export const getMisPagosRepartidor = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: "Error al consultar los pagos del repartidor." });
  }
};
...
// Lista todos los pagos de repartidores.

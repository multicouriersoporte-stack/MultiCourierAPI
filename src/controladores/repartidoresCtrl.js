...
// Campos de usuario.
const usuarioSelect = `u.usuario_nombre, u.usuario_apellido, u.usuario_nombre_completo, u.usuario_telefono, u.usuario_email, u.usuario_foto`;

// Campos de estado.
const estadoSelect = `er.estado_repartidor_nombre, er.estado_repartidor_descripcion, er.estado_repartidor_visible_front, er.estado_repartidor_permite_pedidos, er.estado_repartidor_permite_seleccion`;

// SELECT base del repartidor.
const selectRepartidor = `SELECT r.*, ${usuarioSelect}, ${estadoSelect} FROM repartidores r INNER JOIN usuarios u ON r.id_usuario = u.id_usuario LEFT JOIN estados_repartidor er ON r.id_estado_repartidor = er.id_estado_repartidor`;
...
// Verifica si el repartidor tiene una reserva activa dentro de su horario actual.
const estaDentroDeTurnoActivo = async id_repartidor => {
    const [rows] = await conmysql.query(`
...
// Obtener el primer repartidor disponible con turno reservado vigente.
export const getRepartidorDisponible = async (req, res) => {
    try {
        const [result] = await conmysql.query(`
            SELECT r.*, ${usuarioSelect}, ${estadoSelect}
            ...
            INNER JOIN usuarios u ON r.id_usuario = u.id_usuario
            INNER JOIN estados_repartidor er ON r.id_estado_repartidor = er.id_estado_repartidor
            WHERE r.id_estado_repartidor IN (${ESTADO_REPARTIDOR.LISTO}, ${ESTADO_REPARTIDOR.REPARTIENDO})
            AND er.estado_repartidor_estado = 1
            AND er.estado_repartidor_permite_pedidos = 1
            AND EXISTS (
                SELECT 1
...
export const postRepartidores = async (req, res) => {
    try {
        ...
        const estadoRepartidor = id_estado_repartidor || ESTADO_REPARTIDOR.DESCONECTADO;
        const [estados] = await conmysql.query(`
...
// Cambiar únicamente entre REPARTIENDO y EN_PAUSA durante un turno reservado.
export const cambiarEstadoRepartidor = async (req, res) => {
    try {
        const { id } = req.params;
        const nuevoEstado = Number(req.body.id_estado_repartidor);

        if (![ESTADO_REPARTIDOR.REPARTIENDO, ESTADO_REPARTIDOR.EN_PAUSA].includes(nuevoEstado)) {
            return res.status(400).json({ message: "Solo puedes cambiar entre REPARTIENDO y EN_PAUSA." });
        }

        const [rep] = await conmysql.query(
            `SELECT id_repartidor, id_estado_repartidor FROM repartidores WHERE id_repartidor = ?`, [id]
        );
        if (!rep.length) return res.status(404).json({ message: "Repartidor no encontrado" });

        const estadoActual = Number(rep[0].id_estado_repartidor);
        if ([ESTADO_REPARTIDOR.EN_PEDIDO, ESTADO_REPARTIDOR.DESCONECTADO, ESTADO_REPARTIDOR.INHABILITADO].includes(estadoActual)) {
            return res.status(403).json({ message: "Debes estar en un turno activo (REPARTIENDO o EN_PAUSA) para cambiar tu estado." });
        }

        if (![ESTADO_REPARTIDOR.REPARTIENDO, ESTADO_REPARTIDOR.EN_PAUSA].includes(estadoActual)) {
            return res.status(403).json({ message: "Solo puedes cambiar el estado desde REPARTIENDO o EN_PAUSA." });
        }

        if (!(await estaDentroDeTurnoActivo(id))) {
            return res.status(403).json({ message: "Solo puedes cambiar entre REPARTIENDO y EN_PAUSA mientras estés dentro de tu horario reservado." });
        }
        ...
        await conmysql.query(`UPDATE repartidores SET id_estado_repartidor = ? WHERE id_repartidor = ?`, [nuevoEstado, id]);

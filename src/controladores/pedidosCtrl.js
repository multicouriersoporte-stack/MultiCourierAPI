...
// Obtener rol principal normalizado.
const obtenerRol = (req) => {
    const usuario = req.usuario || {};
    const rol = usuario.usuario_rol ?? usuario.rol_usuario ?? usuario.usuarioRol ?? usuario.rol ?? usuario.role ?? usuario.usuario_role ?? "";
    return String(rol).trim().toUpperCase();
};

// Obtener todos los roles normalizados y sin duplicados.
...
// Comprobar si tiene alguno de los roles indicados.
const tieneRol = (req, rolesPermitidos = []) => {
    const rolesUsuario = obtenerRoles(req);
    const roles = rolesPermitidos.map(rol => String(rol).trim().toUpperCase());
    return rolesUsuario.some(rol => roles.includes(rol));
};

// Comprobar si el usuario es administrativo.
...
const verificarAccesoCliente = async (req, res, id_cliente) => {
    ...
    console.warn("[Pedidos] Rol no autorizado:", { roles: obtenerRoles(req), usuario: req.usuario });
...
// GET: Obtener todos los pedidos según el rol.
export const getPedidos = async (req, res) => {
    try {
        ...
        const id_usuario = obtenerIdUsuario(req);
        console.log("[Pedidos] GET /api/pedidos", { roles, id_usuario });
        ...
        // Los demás roles deben ser administrativos.
        if (!esAdministrativo(req)) return res.status(403).json({
            success: false,
...
// GET: Obtener pedido por ID con sus detalles.
    ...
    try {
        ...
        const [result] = await conmysql.query(`
            SELECT p.*, c.cliente_codigo, l.local_codigo, l.local_nombre_comercial,
                   l.local_razon_social, e.estado_nombre, mp.metodo_pago_nombre,
...
// GET: Obtener pedidos de un local.
export const getPedidosPorLocal = async (req, res) => {
    try {
        ...
        if (!esAdministrativo(req)) return res.status(403).json({ success: false, message: "No tienes permisos para consultar pedidos por local." });
...
// POST: Crear pedido y sus detalles dentro de una transacción.
export const postPedido = async (req, res) => {
    ...
    try {
        ...
        let {
            ...
            pedido_cliente_latitud, pedido_cliente_longitud, pedido_local_latitud,
            pedido_local_longitud, pedido_observacion, id_estado, pedido_fecha,
            pedido_fecha_entrega, productos, detalles
        } = req.body;

        // El cliente siempre usa el cliente asociado a su usuario.
        if (tieneRol(req, ["CLIENTE"])) {
            const id_usuario = obtenerIdUsuario(req);
            if (!id_usuario) return res.status(401).json({ success: false, message: "No se pudo identificar al usuario." });

            const clienteUsuario = await obtenerClienteDelUsuario(id_usuario);
            if (!clienteUsuario) return res.status(403).json({ success: false, message: "El usuario no tiene un cliente asociado." });
            id_cliente = clienteUsuario;
        } else if (!id_cliente) {
            if (!esAdministrativo(req)) return res.status(403).json({ success: false, message: "No tienes permisos para crear pedidos." });
            return res.status(400).json({ success: false, message: "El cliente es obligatorio" });
        }

        if (!id_local) return res.status(400).json({ success: false, message: "El local es obligatorio" });
        ...
        const [locales] = await conexion.query(`SELECT id_local FROM locales WHERE id_local = ? FOR UPDATE`, [id_local]);
        ...
        if (id_metodo_pago !== undefined && id_metodo_pago !== null) {
            const [metodos] = await conexion.query(`SELECT id_metodo_pago FROM metodos_pago WHERE id_metodo_pago = ? AND metodo_pago_estado = 1`, [id_metodo_pago]);
            if (!metodos.length) throw new Error("El método de pago no existe o está inactivo");
        }

        // Generar código consecutivo del pedido.
        ...
        const [result] = await conexion.query(`
            INSERT INTO pedidos (
                pedido_codigo, id_cliente, id_local, id_local_sucursal, id_metodo_pago, pedido_fecha,
                pedido_cantidad_productos, pedido_subtotal_local, pedido_subtotal_app, pedido_adicional_volumen,
        ...
        `, [
            pedido_codigo, id_cliente, id_local, id_local_sucursal ?? null, id_metodo_pago ?? null,
            pedido_fecha ?? new Date(), Number(pedido_cantidad_productos ?? 0), Number(pedido_subtotal_local ?? 0),
            Number(pedido_subtotal_app ?? 0), Number(pedido_adicional_volumen ?? 0), Number(pedido_carrera ?? 0),
            Number(pedido_propina ?? 0), Number(pedido_total ?? 0), Number(pedido_distancia_km ?? 0),
            Number(pedido_tiempo_estimado ?? 0), pedido_cliente_latitud ?? null, pedido_cliente_longitud ?? null,
            pedido_local_latitud ?? null, pedido_local_longitud ?? null, pedido_observacion ?? null,
            id_estado ?? 10, pedido_fecha_entrega ?? null
        ]);

        const id_pedido = result.insertId;
        const detallesRegistrados = [];

        // Validar e insertar cada detalle.
        for (const detalle of listaDetalles) {
            ...
            if (!productosLocal.length) throw new Error(`El producto del local ${id_local_producto} no existe o no pertenece al local del pedido`);
            ...
            if (!Number.isFinite(precioLocal) || precioLocal < 0) throw new Error(`Precio local inválido para el producto ${id_local_producto}`);
            if (!Number.isFinite(precioApp) || precioApp < 0) throw new Error(`Precio app inválido para el producto ${id_local_producto}`);
            ...
            if (!Number.isFinite(subtotalLocal) || subtotalLocal < 0) throw new Error(`Subtotal local inválido para el producto ${id_local_producto}`);
            if (!Number.isFinite(subtotalApp) || subtotalApp < 0) throw new Error(`Subtotal app inválido para el producto ${id_local_producto}`);
            ...
            detallesRegistrados.push({ id_pedido_detalle: detalleResult.insertId, id_local_producto, cantidad, precioLocal, precioApp, subtotalLocal, subtotalApp });
        }

        await conexion.commit();
        return res.status(201).json({
            success: true, id_pedido, pedido_codigo, id_cliente, id_local,
            id_estado: id_estado ?? 10, message: "Pedido registrado con éxito", detalles: detallesRegistrados
        });
    } catch (error) {
        try { await conexion.rollback(); } catch (rollbackError) { console.error("[Pedidos] Error rollback:", rollbackError); }
        console.error("[Pedidos] Error postPedido:", error);

        // Errores de validación se devuelven como 400.
        const mensaje = error.message || "Error al registrar pedido";
        const erroresCliente = ["obligatorio", "no existe", "no está disponible", "debe contener", "cantidad", "producto del local", "Precio", "Subtotal"];
        const esErrorCliente = erroresCliente.some(texto => mensaje.includes(texto));

        return res.status(esErrorCliente ? 400 : 500).json({
            success: false, message: esErrorCliente ? mensaje : "Error al registrar pedido"
        });
    } finally {
        conexion.release();
    }
...
// Campos editables por administradores.
...
// Campos editables por clientes.
const CAMPOS_PEDIDO_CLIENTE = ["pedido_observacion", "pedido_cliente_latitud", "pedido_cliente_longitud", "pedido_fecha_entrega"];

// PUT: Actualizar pedido según permisos del usuario.
export const putPedido = async (req, res) => {
    try {
        ...
        valores.push(id);
        const [result] = await conmysql.query(`UPDATE pedidos SET ${campos.join(", ")} WHERE id_pedido = ?`, valores);
        if (!result.affectedRows) return res.status(404).json({ success: false, message: "Pedido no encontrado" });

        return res.json({ success: true, ...(await obtenerPedidoPorIdInterno(id)) });
    } catch (error) {
...
// PATCH: Actualización parcial de pedido.
export const patchPedido = async (req, res) => {
    try {
        ...
        console.error("[Pedidos] Error patchPedido:", error);
        return res.status(500).json({ success: false, message: "Error al actualizar pedido" });
    }
};

// DELETE: Eliminar pedido y sus detalles; solo administrativos.
export const deletePedido = async (req, res) => {
    ...
    try {
        ...
        if (tieneRol(req, ["CLIENTE"])) return res.status(403).json({ success: false, message: "Los clientes no tienen permisos para eliminar pedidos." });
        if (!esAdministrativo(req)) return res.status(403).json({ success: false, message: "No tienes permisos para eliminar pedidos." });

        await conexion.beginTransaction();

        // Bloquear y eliminar detalles antes del pedido.
        const [pedidos] = await conexion.query(`SELECT id_pedido FROM pedidos WHERE id_pedido = ? FOR UPDATE`, [id]);
        if (!pedidos.length) {
        ...
        await conexion.commit();
        console.log(`[Pedidos] Pedido ${id} eliminado correctamente.`);
        return res.status(204).send();
    } catch (error) {
        try { await conexion.rollback(); } catch (rollbackError) { console.error("[Pedidos] Error rollback deletePedido:", rollbackError); }
...
const obtenerRoles = req => {
    const usuario = req.usuario || {};
    return [obtenerRol(req), ...(Array.isArray(usuario.roles) ? usuario.roles : [])]
        .filter(Boolean)
...
const obtenerLocalDelUsuario = async id_usuario => {
    if (!id_usuario) return null;
    const [locales] = await conmysql.query(`SELECT id_local FROM locales WHERE id_usuario = ? LIMIT 1`, [id_usuario]);
...
// Oculta el PIN a todos excepto al cliente
...
const ocultarPedidosPin = (pedidos, req) => Array.isArray(pedidos) ? pedidos.map(pedido => ocultarPedidoPin(pedido, req)) : pedidos;
...
// Obtiene un pedido con sus relaciones principales
const obtenerPedidoPorIdInterno = async id_pedido => {
    const [pedidos] = await conmysql.query(`
        SELECT p.*, c.cliente_codigo, c.id_usuario AS cliente_id_usuario,
        u.usuario_cedula AS cliente_cedula, u.usuario_nombre AS cliente_nombre,
        ...
        r.repartidor_posicion_ranking, r.repartidor_total_pedidos, r.repartidor_pedidos_aceptados,
        r.repartidor_pedidos_rechazados, r.repartidor_porcentaje_aceptacion,
...
export const getPedidos = async (req, res) => {
    try {
        ...
        if (tieneRol(req, ["REPARTIDOR"])) {
            ...
            const [result] = await conmysql.query(`
                SELECT p.*, c.cliente_codigo, u.usuario_nombre_completo AS cliente_nombre,
                u.usuario_telefono AS cliente_telefono, l.local_codigo, l.local_nombre_comercial,
                e.estado_nombre, mp.metodo_pago_nombre, mp.metodo_pago_descripcion
                FROM pedidos p
                LEFT JOIN clientes c ON p.id_cliente = c.id_cliente
                LEFT JOIN usuarios u ON c.id_usuario = u.id_usuario
        ...
        if (tieneRol(req, ["LOCAL"])) {
            ...
            const id_local = await obtenerLocalDelUsuario(id_usuario);
            if (!id_local) return res.status(403).json({ success: false, message: "El usuario no tiene un local asociado." });
...
export const getPedidoPorId = async (req, res) => {
    try {
        ...
        if (tieneRol(req, ["REPARTIDOR"])) {
            ...
            const id_local = await obtenerLocalDelUsuario(obtenerIdUsuario(req));
            if (Number(pedido.id_local) !== Number(id_local)) return res.status(403).json({ success: false, message: "No puedes acceder a este pedido." });
        } else if (!await verificarAccesoCliente(req, res, pedido.id_cliente)) return;

        const [detalles] = await conmysql.query(`
            SELECT pd.*, p.pedido_codigo, lp.id_local, lp.id_producto, l.local_nombre_comercial,
            pr.producto_codigo, pr.producto_nombre
            FROM pedido_detalles pd
            LEFT JOIN pedidos p ON pd.id_pedido = p.id_pedido
            LEFT JOIN local_productos lp ON pd.id_local_producto = lp.id_local_producto
            LEFT JOIN locales l ON lp.id_local = l.id_local
...
export const getPedidosPorLocal = async (req, res) => {
    try {
        ...
        if (tieneRol(req, ["LOCAL"])) {
            const id_usuario = obtenerIdUsuario(req);
            const localUsuario = await obtenerLocalDelUsuario(id_usuario);

            if (!localUsuario) return res.status(403).json({ success: false, message: "El usuario no tiene un local asociado." });
            if (Number(localUsuario) !== Number(id_local)) return res.status(403).json({ success: false, message: "No puedes consultar pedidos de otro local." });
        ...
        const [result] = await conmysql.query(`
            SELECT p.*, c.cliente_codigo, u.usuario_nombre_completo AS cliente_nombre,
            u.usuario_telefono AS cliente_telefono, l.local_codigo, l.local_nombre_comercial,
            e.estado_nombre, mp.metodo_pago_nombre, r.repartidor_codigo
            FROM pedidos p
            LEFT JOIN clientes c ON p.id_cliente = c.id_cliente
            LEFT JOIN usuarios u ON c.id_usuario = u.id_usuario
            LEFT JOIN locales l ON p.id_local = l.id_local
...
export const postPedido = async (req, res) => {
    ...
    try {
        ...
        if (tieneRol(req, ["CLIENTE"])) {
            ...
            if (!esAdministrativo(req)) {
                conexion.release();
                return res.status(403).json({ success: false, message: "No tienes permisos para crear pedidos." });
            }
        ...
        console.log("[Pedidos] Pedido creado:", { id_pedido, pedido_codigo });

        // Asignación automática
        let asignacion = null;
        try {
            asignacion = await asignarRepartidorAutomaticamente(id_pedido);
            ...
            asignacion = {
                asignado: false,
                motivo: "El pedido fue creado, pero no se pudo realizar la asignación automática."
            };
        }
        ...
        if (transaccionIniciada) {
            try {
                ...
                console.error("[Pedidos] Error rollback:", rollbackError);
            }
        }
        ...
        const erroresCliente = [
            "obligatorio", "no existe", "no está disponible", "debe contener",
            "cantidad", "producto del local", "Precio", "Subtotal", "fecha",
            "método de pago"
        ];

        const esErrorCliente = erroresCliente.some(texto => mensaje.toLowerCase().includes(texto.toLowerCase()));

        return res.status(esErrorCliente ? 400 : 500).json({
            success: false,
            message: esErrorCliente ? mensaje : "Error al registrar pedido"
        });
    } finally {
        conexion.release();
    }
...
const CAMPOS_PEDIDO_ADMIN = [
    ...
    "id_metodo_pago", "pedido_fecha", "pedido_cantidad_productos",
...
export const putPedido = async (req, res) => {
    try {
        ...
        if (tieneRol(req, ["REPARTIDOR"])) {
            ...
            return res.status(403).json({
                success: false,
                message: "El repartidor debe utilizar los endpoints específicos para cambiar el estado del pedido."
            });
        }

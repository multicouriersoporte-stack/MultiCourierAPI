...
import { conmysql } from "../db.js";
import { asignarRepartidorAutomaticamente, obtenerAsignacionActiva } from "./pedidorepartidoresCtrl.js";
import { crearNotificacion, crearNotificacionesMasivas } from "./notificacionesCtrl.js";
import { notificationService } from "../notifications/notification.service.js";
import { procesarEntregaFinanciera } from "../servicios/finanzasService.js";

const emitirEventoPedido = (evento, pedido) => {
  const io = global._io;
  if (!io || !pedido) return;
  if (pedido.id_local) io.to(`local_${pedido.id_local}`).emit(`${evento}_local`, pedido);
  if (pedido.id_pedido) io.to(`pedido_${pedido.id_pedido}`).emit(evento, pedido);
  io.emit(evento, pedido);
};
...
const validarTransicionEstado = async (req, pedido, nuevoIdEstado) => {
  ...
  if (estadoActual === "PENDIENTE" && nuevoEstado === "EN_PREPARACION") {
    ...
    if ((esTransferencia || esTarjeta) && Number(pedido.pedido_pago_confirmado) !== 1) {
      return {
        valido: false, status: 403, codigo: "PAGO_PENDIENTE_CONFIRMACION",
        message: `El pedido utiliza ${esTarjeta ? "TARJETA" : "TRANSFERENCIA"} y el pago todavía no ha sido confirmado por SOPORTE o ADMINISTRADOR.`
      };
    }
  }
...
export const getPedidoDetalles = async (req, res) => {
  try {
    ...
    return res.status(esErrorConexion ? 503 : 500).json({
      success: false,
      message: esErrorConexion ? "El servidor está ocupado procesando muchas solicitudes. Intenta de nuevo en unos segundos." : "Error al obtener los detalles del pedido.",
      codigo: error.code || undefined
    });
  }
};
...
export const putPedido = async (req, res) => {
  try {
    ...
    // Actualización atómica de estado + finanzas cuando se entrega
    const campos = [], valores = [];
    for (const campo of ["id_estado", "pedido_observacion", "pedido_fecha_entrega"]) {
      if (req.body[campo] !== undefined) { campos.push(`${campo}=?`); valores.push(req.body[campo]); }
    }

    let asignacion = null, finanzas = null;
    if (nuevoEstadoNombre === "ENTREGADO") {
      // Estado + pago local + pago repartidor + cobro de efectivo: TODO o NADA.
      const c = await conmysql.getConnection();
      try {
        await c.beginTransaction();
        await c.query(`UPDATE pedidos SET ${campos.join(",")}, pedido_fecha_entrega=COALESCE(pedido_fecha_entrega,NOW()) WHERE id_pedido=?`, [...valores, id]);
        finanzas = await procesarEntregaFinanciera(Number(id), c);
        await c.commit();
      } catch (error) {
        try { await c.rollback(); } catch (_) {}
        console.error("[Pedidos] Error procesando entrega financiera:", error);
        return res.status(error.status || 500).json({ success: false, message: error.message || "No se pudo completar la entrega." });
      } finally { c.release(); }
    } else if (campos.length) {
      await conmysql.query(`UPDATE pedidos SET ${campos.join(",")} WHERE id_pedido=?`, [...valores, id]);
    }
    ...
    return res.json({
      success: true, ...ocultarPedidoPin(pedidoActualizado, req),
      pago_local: finanzas?.pago_local ?? null,
      pago_repartidor: finanzas?.pago_repartidor ?? null,
      finanzas, asignacion
    });
  } catch (error) {
    console.error("[Pedidos] Error putPedido:", error);
...
// Entrega mediante PIN: solo EN_CAMINO, finanzas atómicas, libera repartidor.
export const entregarPedidoConPin = async (req, res) => {
  try {
    const { id } = req.params, { pedido_pin } = req.body;
    if (!esIdValido(id)) return res.status(400).json({ success: false, message: "ID no válido." });
    ...
    if (estadoActual !== "EN_CAMINO")
      return res.status(409).json({ success: false, message: `Solo se puede entregar un pedido EN_CAMINO. Estado actual: ${estadoActual}.` });
    ...
    try {
      await c.beginTransaction();
      const [u] = await c.query(`UPDATE pedidos SET id_estado=?,pedido_fecha_entrega=NOW() WHERE id_pedido=? AND id_estado<>?`, [idEstadoEntregado, id, idEstadoEntregado]);
      if (!u.affectedRows) { await c.rollback(); return res.status(409).json({ success: false, message: "El pedido ya se encuentra ENTREGADO." }); }
      finanzas = await procesarEntregaFinanciera(Number(id), c);
      await c.commit();
    } catch (error) {
      try { await c.rollback(); } catch (_) {}
      console.error("[Pedidos] Error entregando pedido:", error);
      return res.status(error.status || 500).json({ success: false, message: error.message || "Error al entregar pedido." });
    } finally { c.release(); }
    ...
    void notificationService.notifyOrderDelivered(pedidoActualizado, obtenerIdUsuario(req))
      .catch(error => console.error("[Pedidos] Error notificando entrega:", error));
    return res.json({
      success: true, message: "Pedido entregado con éxito.",
      pedido: ocultarPedidoPin(pedidoActualizado, req),
      pago_local: finanzas?.pago_local ?? null,
      pago_repartidor: finanzas?.pago_repartidor ?? null,
      efectivo: finanzas?.efectivo ?? null
    });
  } catch (error) {
    console.error("[Pedidos] Error entregarPedidoConPin:", error);
    return res.status(500).json({ success: false, message: "Error al entregar pedido." });
...
// Cancelación con reglas por rol. Nota: reembolso de billetera pendiente de regla de negocio (3d).
export const cancelarPedido = async (req, res) => {
  const conexion = await conmysql.getConnection();
  try {
    const { id } = req.params;
    ...
    const [pedidos] = await conexion.query(`
      SELECT p.*, e.estado_nombre FROM pedidos p LEFT JOIN estados e ON p.id_estado = e.id_estado
      WHERE p.id_pedido = ? LIMIT 1 FOR UPDATE
    `, [id]);
    ...
    if (!autorizado) {
      await conexion.rollback();
      return res.status(403).json({
        success: false, codigo: "CANCELACION_NO_PERMITIDA",
        message: "Ya no puedes cancelar este pedido en este punto del proceso. Contacta a SOPORTE."
      });
    }
    ...
    await conexion.query(`
      UPDATE pedidos SET id_estado=?, pedido_cancelado_motivo=?, pedido_cancelado_por_rol=?, pedido_cancelado_por_usuario=?, pedido_cancelado_fecha=NOW()
      WHERE id_pedido=?
    `, [idEstadoCancelado, motivo, obtenerRol(req) || null, obtenerIdUsuario(req) || null, id]);

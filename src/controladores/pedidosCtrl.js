...
const obtenerLocalDelUsuario = async id_usuario => {
    ...
    if (!locales.length) {
        console.warn("[Pedidos] Usuario sin local asociado:", id_usuario);
        return null;
    }
...
const obtenerPedidoPorIdInterno = async id_pedido => {
    const [pedidos] = await conmysql.query(
        `SELECT
            ...
            l.local_telefono, l.local_email,
            e.estado_nombre,
...
export const getPedidos = async (req, res) => {
    try {
        ...
        console.log("[Pedidos] Consulta de pedidos:", { id_usuario, roles });
        ...
        if (tieneRol(req, ["LOCAL"])) {
            ...
            if (!local) {
                console.error("[Pedidos][LOCAL] No se encontró local para usuario:", id_usuario);
                return res.status(403).json({
                    success: false,
            ...
            console.log("[Pedidos][LOCAL] Local autenticado:", {
                id_usuario,
                id_local,
                local_codigo: local.local_codigo,
                local_nombre: local.local_nombre_comercial
            });
            ...
            console.log("[Pedidos][LOCAL] Pedidos encontrados:", {
                id_usuario,
                id_local,
...
export const getPedidosPorCliente = async (req, res) => {
    try {
        ...
        return res.status(500).json({
            success: false,
            message: "Error al consultar pedidos del cliente"
        });
    }
};
...
export const getPedidosPorLocal = async (req, res) => {
    try {
        ...
        console.log("[Pedidos][getPedidosPorLocal]", { id_local, cantidad: result.length });
        return res.json(ocultarPedidosPin(result, req));
    } catch (error) {
        console.error("[Pedidos] Error getPedidosPorLocal:", error);
        return res.status(500).json({
            success: false,
            message: "Error al consultar pedidos del local"
        });
    }
};
...
export const getPedidosPorEstado = async (req, res) => {
    try {
        ...
        return res.status(500).json({
            success: false,
            message: "Error al consultar pedidos por estado"
        });
    }
};
    ...
    try {
        ...
        if (tieneRol(req, ["CLIENTE"])) {
            ...
            if (!esAdministrativo(req)) {
                ...
                return res.status(403).json({
                    success: false,
                    message: "No tienes permisos para crear pedidos."
                });
            }
        ...
        if (id_metodo_pago !== undefined && id_metodo_pago !== null) {
            ...
            if (!metodos.length) {
                throw new Error("El método de pago no existe o está inactivo");
            }
        }
        ...
        for (const detalle of listaDetalles) {
            ...
            const cantidad = Number(
                detalle.pedido_detalle_cantidad ?? detalle.cantidad ?? 0
            );
            ...
            const precioLocal = Number(
                detalle.pedido_detalle_precio_local ?? detalle.precioLocal ?? 0
            );

            const precioApp = Number(
                detalle.pedido_detalle_precio_app ?? detalle.precioApp ?? 0
            );
        ...
        console.log("[Pedidos] Pedido creado:", { id_pedido, pedido_codigo, id_local });

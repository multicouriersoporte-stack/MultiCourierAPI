...
/** Local asociado al usuario (por id_usuario o id_local del token) */
const obtenerLocalDelUsuario = async req => {
  ...
  if (id_usuario) {
    const [locales] = await conmysql.query(
      `SELECT id_local, id_usuario, local_codigo, local_nombre_comercial, local_razon_social, local_telefono, local_email, local_latitud, local_longitud
       FROM locales WHERE id_usuario = ? LIMIT 1`, [id_usuario]
    );
    if (locales.length) {
  ...
  if (Number.isInteger(id_local_token) && id_local_token > 0) {
    const [locales] = await conmysql.query(
      `SELECT id_local, id_usuario, local_codigo, local_nombre_comercial, local_razon_social, local_telefono, local_email, local_latitud, local_longitud
       FROM locales WHERE id_local = ? LIMIT 1`, [id_local_token]
    );
    if (locales.length) {
...
// Oculta pedido_pin excepto para CLIENTE
...
const obtenerPedidoPorIdInterno = async id_pedido => {
  const [pedidos] = await conmysql.query(`
    SELECT p.*,
      ...
      r.id_repartidor, r.id_usuario AS repartidor_id_usuario, r.repartidor_codigo, r.repartidor_placa, r.repartidor_tipo_vehiculo,
      r.repartidor_calificacion, r.repartidor_posicion_ranking, r.repartidor_total_pedidos, r.repartidor_pedidos_aceptados,
    ...
    WHERE p.id_pedido = ? LIMIT 1`, [id_pedido]);
...
export const getPedidos = async (req, res) => {
  try {
    ...
    if (tieneRol(req, ["LOCAL"])) {
      console.log("[Pedidos][LOCAL] Iniciando consulta...");
      const local = await obtenerLocalDelUsuario(req);
      if (!local) {
        console.error("[Pedidos][LOCAL] No se encontró local asociado.");
        return res.status(403).json({
          success: false,
          message: "El usuario LOCAL no tiene un registro asociado en la tabla locales.",
          debug: { id_usuario, usuario: req.usuario }
        });
      }
      ...
      console.log("[Pedidos][LOCAL] id_usuario:", id_usuario, "id_local:", id_local, "nombre:", local.local_nombre_comercial);

      const [result] = await conmysql.query(`
        SELECT p.*, c.cliente_codigo,
          u.usuario_nombre AS cliente_nombre, u.usuario_apellido AS cliente_apellido, u.usuario_nombre_completo AS cliente_nombre_completo,
        ...
        LEFT JOIN metodos_pago mp ON p.id_metodo_pago = mp.id_metodo_pago
        LEFT JOIN repartidores r ON p.id_repartidor = r.id_repartidor
        WHERE p.id_local = ? ORDER BY p.id_pedido DESC`, [id_local]);

      console.log(`[Pedidos][LOCAL] Pedidos id_local=${id_local}:`, result.length,
        result.map(p => ({ id_pedido: p.id_pedido, pedido_codigo: p.pedido_codigo, id_local: p.id_local, id_estado: p.id_estado, estado_nombre: p.estado_nombre })));
      return res.json(ocultarPedidosPin(result, req));
    }
    ...
    if (tieneRol(req, ["CLIENTE"])) {
      ...
      const [result] = await conmysql.query(`
        ...
        LEFT JOIN estados e ON p.id_estado = e.id_estado
        LEFT JOIN metodos_pago mp ON p.id_metodo_pago = mp.id_metodo_pago
        WHERE p.id_cliente = ? ORDER BY p.id_pedido DESC`, [id_cliente]);
      return res.json(ocultarPedidosPin(result, req));
    }
    ...
    if (tieneRol(req, ["REPARTIDOR"])) {
      ...
      const [result] = await conmysql.query(`
        ...
        LEFT JOIN estados e ON p.id_estado = e.id_estado
        LEFT JOIN metodos_pago mp ON p.id_metodo_pago = mp.id_metodo_pago
        WHERE p.id_repartidor = ? ORDER BY p.id_pedido DESC`, [id_repartidor]);
      return res.json(ocultarPedidosPin(result, req));
    }
  ...
  try {
    ...
    if (tieneRol(req, ["CLIENTE"])) {
      ...
      const [detalles] = await conmysql.query(`
        SELECT pd.*, p.pedido_codigo, lp.id_local, lp.id_producto, l.local_nombre_comercial, pr.producto_codigo, pr.producto_nombre
        ...
        WHERE pd.id_pedido = ? ORDER BY pd.id_pedido_detalle ASC`, [id]);
...
export const getPedidoPorCodigo = async (req, res) => {
  try {
    ...
    const [result] = await conmysql.query(`
      ...
      LEFT JOIN metodos_pago mp ON p.id_metodo_pago = mp.id_metodo_pago
      LEFT JOIN repartidores r ON p.id_repartidor = r.id_repartidor
      WHERE p.pedido_codigo = ? LIMIT 1`, [String(codigo).trim()]);
  ...
  try {
    ...
    const [result] = await conmysql.query(`
      SELECT p.*, c.cliente_codigo, u.usuario_nombre_completo AS cliente_nombre,
        l.local_codigo, l.local_nombre_comercial, e.estado_nombre, mp.metodo_pago_nombre, r.repartidor_codigo
      FROM pedidos p
      LEFT JOIN clientes c ON p.id_cliente = c.id_cliente
      LEFT JOIN usuarios u ON c.id_usuario = u.id_usuario
      LEFT JOIN locales l ON p.id_local = l.id_local
      ...
      LEFT JOIN metodos_pago mp ON p.id_metodo_pago = mp.id_metodo_pago
      LEFT JOIN repartidores r ON p.id_repartidor = r.id_repartidor
      WHERE p.id_estado = ? ORDER BY p.id_pedido DESC`, [id_estado]);
    return res.json(ocultarPedidosPin(result, req));
  } catch (error) {
...
export const postPedido = async (req, res) => {
  ...
  try {
    ...
    if (tieneRol(req, ["CLIENTE"])) {
      ...
      if (!esAdministrativo(req)) { conexion.release(); return res.status(403).json({ success: false, message: "No tienes permisos para crear pedidos." }); }
      conexion.release();
    ...
    const [result] = await conexion.query(`
      ...
      [
        pedido_codigo, pedido_pin, id_cliente, id_local, id_local_sucursal ?? null, id_metodo_pago ?? null,
        convertirFechaMySQL(pedido_fecha ?? new Date()),
        Number(pedido_cantidad_productos ?? 0), Number(pedido_subtotal_local ?? 0), Number(pedido_subtotal_app ?? 0),
        Number(pedido_adicional_volumen ?? 0), Number(pedido_carrera ?? 0), Number(pedido_propina ?? 0), Number(pedido_total ?? 0),
        Number(pedido_distancia_km ?? 0), Number(pedido_tiempo_estimado ?? 0),
        pedido_cliente_latitud ?? null, pedido_cliente_longitud ?? null, pedido_local_latitud ?? null, pedido_local_longitud ?? null,
    ...
    for (const detalle of listaDetalles) {
      ...
      const [productosLocal] = await conexion.query(`
        ...
        WHERE lp.id_local_producto = ? AND lp.id_local = ? FOR UPDATE`, [id_local_producto, id_local]);
      ...
      if (producto.producto_estado === null || producto.producto_estado === undefined ||
      ...
      const [detalleResult] = await conexion.query(`
        INSERT INTO pedido_detalles (
          ...
          pedido_detalle_precio_app, pedido_detalle_subtotal_local, pedido_detalle_subtotal_app, pedido_detalle_observacion
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      ...
      detallesRegistrados.push({
        id_pedido_detalle: detalleResult.insertId, id_local_producto, cantidad, precioLocal, precioApp, subtotalLocal, subtotalApp
      });
    }
    ...
    const respuesta = {
      ...
      asignacion: { asignado: false, motivo: "La asignación se realizará cuando el local acepte el pedido y lo ponga EN_PREPARACION." },
...
const CAMPOS_PEDIDO_ADMIN = [
  "pedido_codigo", "id_cliente", "id_local", "id_repartidor", "id_local_sucursal", "id_metodo_pago",
  "pedido_fecha", "pedido_cantidad_productos", "pedido_subtotal_local", "pedido_subtotal_app", "pedido_adicional_volumen",
  "pedido_carrera", "pedido_propina", "pedido_total", "pedido_distancia_km", "pedido_tiempo_estimado",
  "pedido_cliente_latitud", "pedido_cliente_longitud", "pedido_local_latitud", "pedido_local_longitud",
  "pedido_observacion", "id_estado", "pedido_fecha_entrega"
...
export const putPedido = async (req, res) => {
  try {
    ...
    // REPARTIDOR → endpoints específicos
    if (tieneRol(req, ["REPARTIDOR"])) {
      const id_repartidor = await obtenerRepartidorDelUsuario(obtenerIdUsuario(req));
      if (Number(pedido.id_repartidor) !== Number(id_repartidor)) {
        return res.status(403).json({ success: false, message: "No puedes modificar este pedido." });
      }
      return res.status(403).json({ success: false, message: "El repartidor debe utilizar los endpoints específicos para cambiar el estado del pedido." });
    }
    ...
    else camposPermitidos = CAMPOS_PEDIDO_ADMIN;

    const campos = [], valores = [];
    let seSolicitaEnPreparacion = false, idEstadoEnPreparacion = null;
    ...
    if (req.body.id_estado !== undefined && req.body.id_estado !== null) {
      ...
      if (!idEstadoEnPreparacion) {
        return res.status(500).json({ success: false, message: 'No existe el estado "EN_PREPARACION" en la tabla estados.' });
      }
      seSolicitaEnPreparacion = Number(req.body.id_estado) === Number(idEstadoEnPreparacion);
    }

    // LOCAL no puede asignar repartidor
    ...
    if (seSolicitaEnPreparacion) {
      console.log("[Pedidos] Pedido EN_PREPARACION. Asignando repartidor:", { id_pedido: Number(id) });
      try {
        asignacion = await asignarRepartidorAutomaticamente(Number(id));
      } catch (errorAsignacion) {
        console.error("[Pedidos] Error asignación:", errorAsignacion);
        asignacion = { asignado: false, motivo: "El pedido quedó EN_PREPARACION, pero no se pudo realizar la asignación automática." };
      }
    }

    const pedidoActualizado = await obtenerPedidoPorIdInterno(id);
    return res.json({ success: true, ...ocultarPedidoPin(pedidoActualizado, req), asignacion });
  } catch (error) {
...
export const deletePedido = async (req, res) => {
  ...
  try {
    ...
    if (!esAdministrativo(req)) { conexion.release(); return res.status(403).json({ success: false, message: "No tienes permisos para eliminar pedidos." }); }

    const pedido = await obtenerPedidoPorIdInterno(id);
    if (!pedido) { conexion.release(); return res.status(404).json({ success: false, message: "Pedido no encontrado" }); }
...
export {
  ...
  obtenerClienteDelUsuario, obtenerLocalDelUsuario, obtenerRepartidorDelUsuario,
  verificarAccesoCliente, obtenerPedidoPorIdInterno, generarPedidoPin,

...
// Obtiene todos los roles del usuario sin duplicados.
...
// Relaciones entre usuario y sus entidades.
...
// El PIN solo se muestra al cliente.
...
// Consulta un pedido con sus relaciones principales.
...
export const getPedidos = async (req, res) => {
    try {
        ...
        if (tieneRol(req, ["REPARTIDOR"])) {
            ...
            if (!id_repartidor) {
                return res.status(403).json({ success: false, message: "El usuario no tiene un repartidor asociado." });
            }
        ...
        // Solo los roles administrativos pueden consultar todos.
...
export const getPedidosPorLocal = async (req, res) => {
    try {
        ...
        // El local solo puede consultar sus propios pedidos.
        if (tieneRol(req, ["LOCAL"])) {
            const id_usuario = obtenerIdUsuario(req);
            ...
            if (Number(localUsuario) !== Number(id_local)) {
...
export const getPedidoPorCodigo = async (req, res) => {
    try {
        ...
        } else if (!await verificarAccesoCliente(req, res, pedido.id_cliente)) {
            return;
        }
...
// POST PEDIDO
// ============================================================
// El pedido se crea sin repartidor. La asignación ocurre al
...
export const postPedido = async (req, res) => {
    ...
    try {
        ...
        // El cliente autenticado siempre usa su propio cliente.
        if (tieneRol(req, ["CLIENTE"])) {
            const id_usuario = obtenerIdUsuario(req);
        ...
        // Validar local y completar coordenadas si faltan.
        ...
        // El estado inicial conserva el comportamiento existente.
        ...
        // Registrar cada producto del pedido.
        for (const detalle of listaDetalles) {
            ...
            if (!Number.isFinite(precioLocal) || precioLocal < 0) {
                throw new Error(`Precio local inválido para el producto ${id_local_producto}`);
            }

            if (!Number.isFinite(precioApp) || precioApp < 0) {
                throw new Error(`Precio app inválido para el producto ${id_local_producto}`);
            }
            ...
            if (!Number.isFinite(subtotalLocal) || subtotalLocal < 0) {
                throw new Error(`Subtotal local inválido para el producto ${id_local_producto}`);
            }

            if (!Number.isFinite(subtotalApp) || subtotalApp < 0) {
                throw new Error(`Subtotal app inválido para el producto ${id_local_producto}`);
            }
        ...
        // La asignación se realiza posteriormente al pasar a EN_PREPARACION.
        ...
        const erroresCliente = [
            "obligatorio", "no existe", "no está disponible",
            "debe contener", "cantidad", "producto del local",
            "Precio", "Subtotal", "fecha", "método de pago"
        ];

        const esErrorCliente = erroresCliente.some(texto =>
            mensaje.toLowerCase().includes(texto.toLowerCase())
        );
...
// Campos que pueden modificarse según el rol.
const CAMPOS_PEDIDO_ADMIN = [
    "pedido_codigo", "id_cliente", "id_local", "id_repartidor",
    "id_local_sucursal", "id_metodo_pago", "pedido_fecha",
    "pedido_cantidad_productos", "pedido_subtotal_local",
    "pedido_subtotal_app", "pedido_adicional_volumen", "pedido_carrera",
    "pedido_propina", "pedido_total", "pedido_distancia_km",
    "pedido_tiempo_estimado", "pedido_cliente_latitud",
    "pedido_cliente_longitud", "pedido_local_latitud",
    "pedido_local_longitud", "pedido_observacion", "id_estado",
...
// PUT / PATCH PEDIDO
// ============================================================
// El local puede cambiar el estado a EN_PREPARACION. En ese
// momento se ejecuta la asignación automática del repartidor.
export const putPedido = async (req, res) => {
    try {
        ...
        // Los repartidores usan endpoints específicos.
        if (tieneRol(req, ["REPARTIDOR"])) {
            const id_repartidor = await obtenerRepartidorDelUsuario(obtenerIdUsuario(req));
        ...
        // Validar acceso del cliente o propietario del local.
        if (tieneRol(req, ["CLIENTE"])) {
            if (!await verificarAccesoCliente(req, res, pedido.id_cliente)) return;
        } else if (tieneRol(req, ["LOCAL"])) {
            const id_local = await obtenerLocalDelUsuario(obtenerIdUsuario(req));
        ...
        // Detectar si el nuevo estado es EN_PREPARACION.
        ...
        if (req.body.id_estado !== undefined && req.body.id_estado !== null) {
            ...
            if (!idEstadoEnPreparacion) {
                return res.status(500).json({
                    success: false,
                    message: 'No existe el estado "EN_PREPARACION" en la tabla estados.'
                });
            }

            seSolicitaEnPreparacion =
                Number(req.body.id_estado) === Number(idEstadoEnPreparacion);
        }

        // El local no puede elegir manualmente al repartidor.
        ...
        // Asignar repartidor inmediatamente al entrar en EN_PREPARACION.
        let asignacion = null;

        if (seSolicitaEnPreparacion) {
            console.log("[Pedidos] Pedido EN_PREPARACION. Iniciando asignación automática:", {
                id_pedido: Number(id)
            });
...
export const deletePedido = async (req, res) => {
    ...
    try {
        ...
        await conexion.query(`
            DELETE FROM pedido_repartidores WHERE id_pedido = ?
        `, [id]);

        await conexion.query(`
            DELETE FROM pedido_detalles WHERE id_pedido = ?
        `, [id]);

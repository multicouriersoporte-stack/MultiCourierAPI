...
const obtenerPedidoPorIdInterno = async id_pedido => {
  const [pedidos] = await conmysql.query(`
    SELECT p.*, c.cliente_codigo, c.id_usuario AS cliente_id_usuario, u.usuario_cedula AS cliente_cedula,
           u.usuario_nombre AS cliente_nombre, u.usuario_apellido AS cliente_apellido, u.usuario_nombre_completo AS cliente_nombre_completo,
           u.usuario_email AS cliente_email, u.usuario_telefono AS cliente_telefono, l.local_codigo, l.local_nombre_comercial,
           l.local_razon_social, l.local_telefono, l.local_email, e.estado_nombre, mp.metodo_pago_nombre, mp.metodo_pago_descripcion,
...
export const getPedidos = async (req, res) => {
  try {
    ...
    if (tieneRol(req, ["CLIENTE"])) {
      ...
      if (!id_cliente) return res.json([]);
      const [result] = await conmysql.query(`
        SELECT p.*, c.cliente_codigo, l.local_codigo, l.local_nombre_comercial, l.local_razon_social,
               l.local_telefono, e.estado_nombre, mp.metodo_pago_nombre, mp.metodo_pago_descripcion
        FROM pedidos p
        LEFT JOIN clientes c ON p.id_cliente = c.id_cliente
        LEFT JOIN locales l ON p.id_local = l.id_local
        LEFT JOIN estados e ON p.id_estado = e.id_estado
...
export const postPedido = async (req, res) => {
  ...
  try {
    ...
    let {
      id_cliente, id_local, id_local_sucursal, id_metodo_pago, pedido_cantidad_productos,
      pedido_subtotal_local, pedido_subtotal_app, pedido_adicional_volumen, pedido_carrera,
      pedido_propina, pedido_total, pedido_distancia_km, pedido_tiempo_estimado,
      pedido_cliente_latitud, pedido_cliente_longitud, pedido_local_latitud, pedido_local_longitud,
      pedido_observacion, id_estado, pedido_fecha, pedido_fecha_entrega, productos, detalles
    } = req.body;
    ...
    const [result] = await conexion.query(`
      INSERT INTO pedidos (
        pedido_codigo, pedido_pin, id_cliente, id_local, id_repartidor, id_local_sucursal, id_metodo_pago,
        pedido_fecha, pedido_cantidad_productos, pedido_subtotal_local, pedido_subtotal_app, pedido_adicional_volumen,
        pedido_carrera, pedido_propina, pedido_total, pedido_distancia_km, pedido_tiempo_estimado,
        pedido_cliente_latitud, pedido_cliente_longitud, pedido_local_latitud, pedido_local_longitud,
        pedido_observacion, id_estado, pedido_fecha_entrega, pedido_pago_confirmado
      ) VALUES (?, ?, ?, ?, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      pedido_codigo, pedido_pin, id_cliente, id_local, id_local_sucursal ?? null, id_metodo_pago,
      convertirFechaMySQL(pedido_fecha ?? new Date()), Number(pedido_cantidad_productos ?? 0),
      Number(pedido_subtotal_local ?? 0), Number(pedido_subtotal_app ?? 0), Number(pedido_adicional_volumen ?? 0),
      ...
      pedido_cliente_latitud ?? null, pedido_cliente_longitud ?? null, pedido_local_latitud ?? null,
      pedido_local_longitud ?? null, pedido_observacion ?? null, estadoInicial, pedido_fecha_entrega ?? null,
      pagoConfirmadoInicial
    ]);
    ...
    for (const detalle of listaDetalles) {
      ...
      const [productosLocal] = await conexion.query(`
        SELECT lp.id_local_producto, pr.producto_nombre, pr.producto_estado
        FROM local_productos lp INNER JOIN productos pr ON lp.id_producto = pr.id_producto
        WHERE lp.id_local_producto = ? AND lp.id_local = ? FOR UPDATE
      `, [id_local_producto, id_local]);
      ...
      const [detalleResult] = await conexion.query(`
        INSERT INTO pedido_detalles (
          id_pedido, id_local_producto, pedido_detalle_cantidad, pedido_detalle_precio_local,
          pedido_detalle_precio_app, pedido_detalle_subtotal_local, pedido_detalle_subtotal_app,
          pedido_detalle_observacion
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `, [
        id_pedido, id_local_producto, cantidad, precioLocal, precioApp, subtotalLocal, subtotalApp,
        detalle.pedido_detalle_observacion ?? detalle.observacion ?? null
      ]);
    ...
    return res.status(201).json({
      success: true, id_pedido, pedido_codigo, id_cliente, id_local,
      id_repartidor: pedidoFinal?.id_repartidor ?? null, id_estado: pedidoFinal?.id_estado ?? estadoInicial,
      estado_nombre: pedidoFinal?.estado_nombre ?? null, id_metodo_pago: pedidoFinal?.id_metodo_pago ?? id_metodo_pago,
      metodo_pago_nombre: pedidoFinal?.metodo_pago_nombre ?? null,
      pedido_pago_confirmado: Number(pedidoFinal?.pedido_pago_confirmado ?? pagoConfirmadoInicial),
      message: esTransferencia
        ? "Pedido registrado. La transferencia queda pendiente de confirmación por SOPORTE o ADMINISTRADOR."
...
export const putPedido = async (req, res) => {
  try {
    ...
    const campos = [], valores = [], camposPermitidos = ["id_estado", "pedido_observacion", "pedido_fecha_entrega"];
    for (const campo of camposPermitidos) {
      if (req.body[campo] !== undefined) {
        campos.push(`${campo} = ?`);
...
export const patchPedido = (req, res) => putPedido(req, res);

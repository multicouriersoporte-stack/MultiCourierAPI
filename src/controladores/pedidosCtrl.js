...
export const getPedidoPorId = async (req, res) => {
    try {
        ...
        if (tieneRol(req, ["CLIENTE"])) {
            ...
            if (Number(pedido.id_local) !== Number(id_local)) {
                return res.status(403).json({
                    success: false,
                    message: "No puedes acceder a este pedido."
                });
...
// Obtiene todos los roles únicos del usuario.
const obtenerRoles = req => {
    const usuario = req.usuario || {};
    return [obtenerRol(req), ...(Array.isArray(usuario.roles) ? usuario.roles : [])]
        .filter(Boolean)
...
// Oculta el PIN excepto para el cliente propietario.
const ocultarPedidoPin = (pedido, req) => {
    if (!pedido) return pedido;
    const copia = { ...pedido };
    if (!tieneRol(req, ["CLIENTE"])) delete copia.pedido_pin;
...
const obtenerPedidoPorIdInterno = async id_pedido => {
    const [pedidos] = await conmysql.query(
        `SELECT p.*, c.cliente_codigo, c.id_usuario AS cliente_id_usuario,
            ...
            r.repartidor_codigo, r.repartidor_placa, r.repartidor_tipo_vehiculo,
            r.repartidor_calificacion, r.repartidor_posicion_ranking,
            r.repartidor_total_pedidos, r.repartidor_pedidos_aceptados,
            r.repartidor_pedidos_rechazados, r.repartidor_porcentaje_aceptacion,
            ur.usuario_nombre AS repartidor_nombre, ur.usuario_apellido AS repartidor_apellido,
            ur.usuario_nombre_completo AS repartidor_nombre_completo, ur.usuario_telefono AS repartidor_telefono
        FROM pedidos p
...
export const getPedidos = async (req, res) => {
    try {
        ...
        // LOCAL: solo pedidos de su local.
        if (tieneRol(req, ["LOCAL"])) {
            if (!id_usuario) return res.status(401).json({ success: false, message: "No se pudo identificar al usuario." });
            const id_local = await obtenerLocalDelUsuario(id_usuario);
            if (!id_local) return res.status(403).json({ success: false, message: "El usuario no tiene un local asociado." });
        ...
        // Solo administración puede consultar todos.
        if (!esAdministrativo(req)) {
            return res.status(403).json({
                success: false,
                message: `Los roles [${roles.join(", ") || "SIN_ROL"}] no tienen permisos para consultar pedidos.`
...
export const getPedidoPorId = async (req, res) => {
    try {
        ...
        // Cada rol solo puede consultar sus propios pedidos.
        if (tieneRol(req, ["CLIENTE"])) {
            if (!await verificarAccesoCliente(req, res, pedido.id_cliente)) return;
        } else if (tieneRol(req, ["REPARTIDOR"])) {
            const id_repartidor = await obtenerRepartidorDelUsuario(obtenerIdUsuario(req));
...
export const getPedidosPorLocal = async (req, res) => {
    try {
        ...
        // LOCAL solo puede consultar su propio local.
        if (tieneRol(req, ["LOCAL"])) {
            const id_usuario = obtenerIdUsuario(req);
            const localUsuario = await obtenerLocalDelUsuario(id_usuario);
            if (!localUsuario) return res.status(403).json({ success: false, message: "El usuario no tiene un local asociado." });
...
export const getPedidoPorCodigo = async (req, res) => {
    try {
        ...
        // Valida que repartidor, local o cliente sean propietarios del pedido.
        if (tieneRol(req, ["REPARTIDOR"])) {
            const id_repartidor = await obtenerRepartidorDelUsuario(obtenerIdUsuario(req));
            if (Number(pedido.id_repartidor) !== Number(id_repartidor)) return res.status(403).json({ success: false, message: "No puedes acceder a este pedido." });
        } else if (tieneRol(req, ["LOCAL"])) {
...
export const postPedido = async (req, res) => {
    ...
    try {
        ...
        // El cliente siempre crea pedidos a su propio nombre.
        if (tieneRol(req, ["CLIENTE"])) {
            const id_usuario = obtenerIdUsuario(req);
            if (!id_usuario) {
                conexion.release();
        ...
        // Genera código secuencial y PIN.
        ...
        // El repartidor se asigna cuando el local pase el pedido a EN_PREPARACION.
        const [result] = await conexion.query(
            `INSERT INTO pedidos (
                pedido_codigo, pedido_pin, id_cliente, id_local, id_repartidor,
        ...
        // Registra y valida cada producto.
        for (const detalle of listaDetalles) {
            const id_local_producto = Number(detalle.id_local_producto);
            const cantidad = Number(detalle.pedido_detalle_cantidad ?? detalle.cantidad ?? 0);
            ...
            detallesRegistrados.push({
                id_pedido_detalle: detalleResult.insertId, id_local_producto, cantidad,
                precioLocal, precioApp, subtotalLocal, subtotalApp
            });
        }
        ...
        const respuesta = {
            ...
            id_repartidor: pedidoFinal?.id_repartidor ?? null,
            id_estado: pedidoFinal?.id_estado ?? estadoInicial,
            message: "Pedido registrado con éxito. Queda pendiente de aceptación por el local.",
            asignacion: {
                asignado: false,
                motivo: "El repartidor se asignará automáticamente cuando el local acepte el pedido y lo ponga EN_PREPARACION."
            },
            detalles: detallesRegistrados
        };
        ...
        const erroresCliente = [
            "obligatorio", "no existe", "no está disponible", "debe contener",
            "cantidad", "producto del local", "Precio", "Subtotal", "fecha", "método de pago"
        ];
        const esErrorCliente = erroresCliente.some(texto => mensaje.toLowerCase().includes(texto.toLowerCase()));
...
const CAMPOS_PEDIDO_ADMIN = [
    ...
    "pedido_total", "pedido_distancia_km", "pedido_tiempo_estimado",
    "pedido_cliente_latitud", "pedido_cliente_longitud", "pedido_local_latitud",
    "pedido_local_longitud", "pedido_observacion", "id_estado", "pedido_fecha_entrega"
];

const CAMPOS_PEDIDO_CLIENTE = [
    "pedido_observacion", "pedido_cliente_latitud",
    "pedido_cliente_longitud", "pedido_fecha_entrega"
];

const CAMPOS_PEDIDO_LOCAL = ["id_estado"];
...
export const putPedido = async (req, res) => {
    try {
        ...
        // REPARTIDOR: debe usar endpoints específicos.
        if (tieneRol(req, ["REPARTIDOR"])) {
            const id_repartidor = await obtenerRepartidorDelUsuario(obtenerIdUsuario(req));
        ...
        // LOCAL solo modifica el estado de sus pedidos.
        if (tieneRol(req, ["LOCAL"])) {
            const id_usuario = obtenerIdUsuario(req);
            const id_local = await obtenerLocalDelUsuario(id_usuario);
            if (!id_local) return res.status(403).json({ success: false, message: "El usuario no tiene un local asociado." });
            if (Number(pedido.id_local) !== Number(id_local)) return res.status(403).json({ success: false, message: "No puedes modificar pedidos de otro local." });
        ...
        // Solo LOCAL -> EN_PREPARACION activa la asignación automática.
        let debeAsignarRepartidor = false;
        let nombreEstadoNuevo = null;

        if (req.body.id_estado !== undefined) {
            const id_estado_nuevo = Number(req.body.id_estado);
            if (!esIdValido(id_estado_nuevo)) return res.status(400).json({ success: false, message: "El ID del estado no es válido." });
            ...
            if (!estados.length) return res.status(400).json({ success: false, message: "El estado indicado no existe." });

            nombreEstadoNuevo = String(estados[0].estado_nombre || "").trim().toUpperCase();

            if (
                tieneRol(req, ["LOCAL"]) &&
                nombreEstadoNuevo === "EN_PREPARACION" &&
                String(pedido.estado_nombre || "").trim().toUpperCase() !== "EN_PREPARACION" &&
                !pedido.id_repartidor
            ) debeAsignarRepartidor = true;
        }
        ...
        // Asigna repartidor después de cambiar a EN_PREPARACION.
        if (debeAsignarRepartidor) {
            try {
                console.log(`[Pedidos] Pedido ${id}: estado EN_PREPARACION. Iniciando asignación automática.`);
                asignacion = await asignarRepartidorAutomaticamente(id);
                console.log(`[Pedidos] Pedido ${id}: resultado asignación:`, asignacion);
            } catch (errorAsignacion) {
                console.error(`[Pedidos] Pedido ${id}: error en asignación automática:`, errorAsignacion);
                asignacion = {
                    asignado: false,
                    motivo: "El pedido pasó a EN_PREPARACION, pero no se pudo realizar la asignación automática.",
                    error: errorAsignacion.message
                };
            }
        }

        const pedidoActualizado = await obtenerPedidoPorIdInterno(id);
        const respuesta = { success: true, ...ocultarPedidoPin(pedidoActualizado, req) };

        if (debeAsignarRepartidor) {
            respuesta.asignacion = asignacion;

            if (asignacion?.asignado) {
                respuesta.message = "Pedido actualizado a EN_PREPARACION y repartidor asignado correctamente.";
                respuesta.notificacion_repartidor = {
                    tipo: "PEDIDO_ASIGNADO",
                    titulo: "Pedido Asignado",
                    mensaje: "Tienes un nuevo pedido asignado. Dirígete al local para recogerlo.",
                    id_pedido: Number(id),
                    pedido_codigo: pedidoActualizado?.pedido_codigo ?? null,
                    id_repartidor: pedidoActualizado?.id_repartidor ?? null
                };
            } else {
                respuesta.message = "Pedido actualizado a EN_PREPARACION, pero no fue posible asignar un repartidor.";
            }
        } else {
...
// PATCH usa la misma lógica que PUT.
export const patchPedido = async (req, res) => putPedido(req, res);
...
export const deletePedido = async (req, res) => {
    ...
    try {
        ...
        // Elimina primero relaciones y detalles.
        await conexion.query(`DELETE FROM pedido_repartidores WHERE id_pedido = ?`, [id]);
        await conexion.query(`DELETE FROM pedido_detalles WHERE id_pedido = ?`, [id]);

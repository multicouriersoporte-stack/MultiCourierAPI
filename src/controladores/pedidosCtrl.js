...
// Valida que el rol pueda ejecutar la transición solicitada.
...
// El PIN solo es visible para el cliente.
...
// Verifica que el cliente consulte únicamente sus propios pedidos.
...
// Consulta completa de un pedido.
...
export const getPedidoPorId = async (req, res) => {
  try {
    ...
    // Cliente y local reciben también el detalle.
    if (tieneRol(req, ["CLIENTE"])) {
...
export const postPedido = async (req, res) => {
  ...
  try {
    ...
    // El cliente siempre se obtiene desde el token.
    const id_usuario = obtenerIdUsuario(req);
    if (!id_usuario) {
      conexion.release();
    ...
    // Valida el método de pago si fue enviado.
    ...
    // Genera el código consecutivo y PIN.
    ...
    /*         let estadoInicial = id_estado;
            if (estadoInicial === undefined || estadoInicial === null) {
                estadoInicial = await obtenerIdEstadoPorNombre("PENDIENTE");
            }
            if (!estadoInicial) throw new Error('No existe el estado "PENDIENTE" en la tabla estados.'); */

    // Los pedidos nuevos SIEMPRE nacen en PENDIENTE.
    // El cliente no puede elegir el estado inicial.
    const estadoInicial = await obtenerIdEstadoPorNombre("PENDIENTE");

    if (!estadoInicial) {
      throw new Error('No existe el estado "PENDIENTE" en la tabla estados.');
    }
    ...
    const [result] = await conexion.query(
      `INSERT INTO pedidos (
        ...
        pedido_cliente_longitud,pedido_local_latitud,pedido_local_longitud,pedido_observacion,id_estado,pedido_fecha_entrega
      ) VALUES (?,?,?,?,NULL,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [
    ...
    for (const detalle of listaDetalles) {
      ...
      detallesRegistrados.push({
        ...
        id_local_producto, cantidad, precioLocal, precioApp, subtotalLocal, subtotalApp
      });
    }
    ...
    return res.status(201).json({
      success: true,
      id_pedido, pedido_codigo, id_cliente, id_local,
...
// Campos modificables por cada rol.
const CAMPOS_PEDIDO_ADMIN = [
  "pedido_codigo", "id_cliente", "id_local", "id_repartidor", "id_local_sucursal", "id_metodo_pago", "pedido_fecha",
  "pedido_cantidad_productos", "pedido_subtotal_local", "pedido_subtotal_app", "pedido_adicional_volumen",
  "pedido_carrera", "pedido_propina", "pedido_total", "pedido_distancia_km", "pedido_tiempo_estimado",
  "pedido_cliente_latitud", "pedido_cliente_longitud", "pedido_local_latitud", "pedido_local_longitud",
...
const CAMPOS_PEDIDO_CLIENTE = ["pedido_observacion", "id_estado", "pedido_cliente_latitud", "pedido_cliente_longitud", "pedido_fecha_entrega"];
...
export const putPedido = async (req, res) => {
  try {
    ...
    // CENTRAL y SUPERVISOR solo consultan.
    ...
    // El repartidor solo puede cambiar el estado de sus pedidos.
    if (tieneRol(req, ["REPARTIDOR"])) {
      const id_repartidor = await obtenerRepartidorDelUsuario(obtenerIdUsuario(req));
      if (Number(pedido.id_repartidor) !== Number(id_repartidor)) {
      ...
      if (camposNoPermitidos.length) {
        return res.status(403).json({
          success: false,
          message: "El repartidor solo puede modificar el estado del pedido."
        });
      }
    }

    // El cliente solo modifica sus propios pedidos.
    if (tieneRol(req, ["CLIENTE"])) {
      if (!(await verificarAccesoCliente(req, res, pedido.id_cliente))) return;
    ...
    // El local solo modifica sus pedidos y no asigna repartidores.
    if (tieneRol(req, ["LOCAL"])) {
      const local = await obtenerLocalDelUsuario(req);
      if (!local) return res.status(403).json({ success: false, message: "El usuario no tiene un local asociado." });
      if (Number(pedido.id_local) !== Number(local.id_local)) {
    ...
    // Valida la transición antes de actualizar.
    /*     if (req.body.id_estado !== undefined && req.body.id_estado !== null) {
          ...
          seSolicitaEnPreparacion = validacionEstado.nuevoEstado === "EN_PREPARACION";
          ...
          seSolicitaEnPreparacion = transicionAEnPreparacion;
        } */
    ...
    /*         const [result] = await conmysql.query(
            ...
            // Al entrar en preparación se intenta asignar automáticamente.
            let asignacion = null;
    ...
    // PENDIENTE -> EN_PREPARACION: crear pago local y asignar repartidor.
    /*     let pagoLocal = null;
        let asignacion = null;
    
        if (transicionAEnPreparacion) {
          console.log("[Pedidos] PENDIENTE -> EN_PREPARACION:", {
            id_pedido: Number(id),
            id_local: pedido.id_local
          });
    
          // Generar pago local del 5%; no se duplica si ya existe.
          try {
            ...
            pagoLocal = {
              ...
              existente: false,
              error: true,
              motivo: "El pedido quedó EN_PREPARACION, pero no se pudo generar el pago local."
            };
          }
    ...
    // EN_CAMINO -> ENTREGADO:
    // generar pago local del 5%.
    if (transicionAEntregado) {
      console.log("[Pedidos] Pedido ENTREGADO. Generando pago local:", {
        id_pedido: Number(id),
        id_local: pedido.id_local
      });
...
export const deletePedido = async (req, res) => {
  ...
  try {
    ...
    // Solo soporte y administrador pueden eliminar.
